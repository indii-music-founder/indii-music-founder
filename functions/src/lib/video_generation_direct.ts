...
export async function generateVideoDirect(params: DirectVideoGenerationParams): Promise<void> {
    ...
    try {
        ...
        const projectId = process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT || 'YOUR_FIREBASE_PROJECT_ID';
        ...
        const targetBucketName = (process.env.VITE_FIREBASE_STORAGE_BUCKET || 'YOUR_FIREBASE_ELECTRON_APP_ID.appspot.com').replace('.appspot.com', '');
