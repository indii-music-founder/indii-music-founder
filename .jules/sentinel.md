...
**Prevention:** Implemented `validateSafeVideoOutputPath` which enforces that output paths are within specific allowed directories (Documents/indii, UserData, Temp), use allowed extensions (.mp4, .webm, .mov), and do not target system roots. Applied this validation in the `video:render` handler.
...
## 2026-02-20 - [MEDIUM] Unsanitized PII in AI Service Debug Logs
**Vulnerability:** The `FirebaseAIService.generateContent` method logged the raw, unsanitized user prompt to the console for debugging purposes. This allowed sensitive information (e.g., passwords, credit card numbers) contained in the prompt to be written to persistent logs (`logs/main.log`) or visible in the developer tools, potentially leading to data leaks.
**Learning:** Logging raw inputs for "deep diagnosis" is a common trap. While helpful for debugging, it bypasses downstream sanitization logic. Security-critical sanitization must occur *before* any data crosses a trust boundary, including writing to logs.
**Prevention:** Modified `FirebaseAIService.ts` to explicitly call `sanitizePrompt` on the input payload *before* passing it to `console.log`. This ensures that PII is redacted in the logs while still providing useful debug information.
