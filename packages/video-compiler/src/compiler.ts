...
/**
 * Vendored font binaries per family, keyed by the css2 weight descriptor the
 * snapshot was fetched with (e.g. `400`, or `400 900` for variable fonts).
 * Values are raw woff2 base64 (no data-URI prefix). Executors load these from
 * `@indii/video-compiler/fontAssets` (generated by
 * `scripts/vendor-video-fonts.mjs`) and inject them via `CompilerOptions` —
 * the pure compiler itself carries no binary assets.
 */
export type EmbeddedFontAssets = Record<string, Record<string, string>>;

export interface CompilerOptions {
    /**
     * When provided, the compiler emits `@font-face` data-URI rules for the
     * text families a project actually uses, making the composition render
     * with real typography everywhere (web preview srcdoc, desktop preview,
     * cloud render) with zero network access. Families without an asset entry
     * keep the sans-serif fallback.
     */
    fontAssets?: EmbeddedFontAssets;
}

/** UI weight keywords → numeric weight for asset matching. */
const normalizeWeight = (raw: string | undefined): number => {
    switch (raw) {
        case undefined: return 700;
        case 'normal': return 400;
        case 'bold': return 700;
        default: {
            const n = Number(raw);
            return Number.isFinite(n) && n >= 100 && n <= 900 ? n : 700;
        }
    }
};

interface FontFaceCandidate { descriptor: string; min: number; max: number; base64: string; }

const parseDescriptor = (descriptor: string): FontFaceCandidate | null => {
    const parts = descriptor.trim().split(/\s+/).map(Number);
    if (parts.length === 0 || parts.some(n => !Number.isFinite(n))) return null;
    return { descriptor, min: Math.min(...parts), max: Math.max(...parts), base64: '' };
};

/** Deterministic nearest-weight asset pick: exact range → closest below → closest above → first. */
export const selectFontAsset = (
    assets: NonNullable<CompilerOptions['fontAssets']>[string],
    weight: number,
): { descriptor: string; base64: string } | null => {
    const candidates = Object.entries(assets)
        .map(([descriptor, base64]) => ({ ...parseDescriptor(descriptor), base64 }) as FontFaceCandidate)
        .filter((c): c is FontFaceCandidate => c !== null)
        .sort((a, b) => a.min - b.min || a.max - b.max);
    if (candidates.length === 0) return null;
    const containing = candidates.find(c => weight >= c.min && weight <= c.max);
    if (containing) return containing;
    const below = candidates.filter(c => c.max < weight).pop();
    if (below) return below;
    const above = candidates.find(c => c.min > weight);
    return above ?? candidates[0]!;
};

/** @font-face data-URI rules for the families/weights a project uses. */
const fontFaceCssFor = (
    used: Map<string, Set<number>>,
    fontAssets: NonNullable<CompilerOptions['fontAssets']>,
): string => {
    const rules: string[] = [];
    for (const family of [...used.keys()].sort()) {
        const familyAssets = fontAssets[family];
        if (!familyAssets) continue;
        const emitted = new Set<string>();
        for (const weight of [...used.get(family)!].sort((a, b) => a - b)) {
            const asset = selectFontAsset(familyAssets, weight);
            if (!asset || emitted.has(asset.descriptor)) continue;
            emitted.add(asset.descriptor);
            rules.push(
                `@font-face{font-family:'${family}';font-style:normal;font-weight:${asset.descriptor};src:url(data:font/woff2;base64,${asset.base64}) format('woff2');}`,
            );
        }
    }
    return rules.join('\n');
...
export const compileProjectToHyperFrames = (
    ...
    // Typography contract: collect the (family, weight) pairs text clips use
    // so @font-face rules can cover exactly what renders — nothing more.
    ...
    for (const [clipIndex, clip] of project.clips.entries()) {
        if (clip.type === 'text') {
            const family = resolveTextFontFamily(clip.fontFamily);
            const weightSet = usedFonts.get(family) ?? new Set<number>();
            weightSet.add(normalizeWeight(clip.fontWeight));
            usedFonts.set(family, weightSet);
        }
    ...
    const fontFaceCss = options.fontAssets && usedFonts.size > 0
        ? fontFaceCssFor(usedFonts, options.fontAssets)
        : '';
...
<style>
  ...
  .clip { position:absolute; }${fontFaceBlock}
</style></head>
<body>
