...
const DISTRIBUTOR_LEAD_HOURS: Record<string, number> = {
    distrokid: 48,
    tunecore: 72,
    cdbaby: 240,   // 10 days
...
async function checkDistributorStatus(
    ...
    if (!apiKey || !distributorReleaseId) return null;
...
function applyStatusHeuristic(
    ...
    if (!deliveredAtMs) return currentStatus;

    const hoursSinceDelivery = (Date.now() - deliveredAtMs) / (1000 * 60 * 60);
    const leadHours = DISTRIBUTOR_LEAD_HOURS[distributorId] ?? 72;

    if (currentStatus === 'pending' || currentStatus === 'pending_review') {
        if (hoursSinceDelivery >= 2) return 'in_review';
    }
...
export const pollDeliveryStatus = onSchedule({
    ...
    try {
        ...
        if (snapshot.empty) {
            logger.info('[pollDeliveryStatus] No pending deliveries to check.');
            return;
        }

        logger.info(`[pollDeliveryStatus] Checking ${snapshot.size} pending releases.`);
        ...
        for (const doc of snapshot.docs) {
            ...
            const apiStatus = await checkDistributorStatus(
                data.distributorId,
                data.distributorReleaseId || '',
                data.apiKey
            );

            const newStatus = apiStatus ?? applyStatusHeuristic(
                data.deliveryStatus,
                data.distributorId,
                data.deliveredAt
            );
            ...
            if (newStatus !== data.deliveryStatus || !data.lastStatusCheck) {
                updates.push(
                    doc.ref.update({
                        ...
                        logger.info(
                            `[pollDeliveryStatus] ${doc.id} (${data.distributorId}): ${data.deliveryStatus} → ${newStatus}`
                        );
                    })
                );
            } else {
                ...
                updates.push(
                    doc.ref.update({ lastStatusCheck: now }).then(() => undefined)
                );
            }
        }
