...
import { queueReleaseCanvasRender } from '../queueReleaseCanvasRender.js';
...
describe('queueReleaseCanvasRender MCP tool', () => {
    beforeEach(() => {
    ...
    it('writes a whitelisted mcpRenderJobs doc, dropping unknown animationSpec keys, and dispatches to Inngest', async () => {
        ...
        const result = await queueReleaseCanvasRender.handler({
            releaseId: 'rel-1',
            canvasType: 'Spotify',
            animationSpec: {
                template: 'waveform',
    ...
    it('rejects invalid canvasType and out-of-range durationSeconds is dropped', async () => {
        const bad = await queueReleaseCanvasRender.handler(
        ...
        releaseGetMock.mockResolvedValueOnce({ exists: true, data: () => ({}) });
        const ok = await queueReleaseCanvasRender.handler(
