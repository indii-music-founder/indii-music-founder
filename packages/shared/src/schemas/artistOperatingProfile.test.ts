...
import {
    ...
    hasAutonomousComputerControl,
} from './artistOperatingProfile';

describe('ArtistOperatingProfileSchema', () => {
    it('accepts the default profile', () => {
        expect(ArtistOperatingProfileSchema.parse(DEFAULT_ARTIST_OPERATING_PROFILE)).toEqual(
            DEFAULT_ARTIST_OPERATING_PROFILE,
        );
    });
    ...
    it('accepts a fully populated profile', () => {
        const profile = {
            schemaVersion: 'artist-operating-profile.v1' as const,
    ...
    it('rejects unknown top-level fields (strict schema)', () => {
        expect(() =>
            ArtistOperatingProfileSchema.parse({
    ...
    it('rejects the wrong schemaVersion literal', () => {
        expect(() =>
            ArtistOperatingProfileSchema.parse({ schemaVersion: 'v2', businessGoals: [] }),
        ).toThrow();
    });
