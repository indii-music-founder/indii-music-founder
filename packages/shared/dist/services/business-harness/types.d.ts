export type HarnessDomain = 'artist_memory' | 'song_dna' | 'creator_protection' | 'distribution_ddex' | 'release' | 'finance' | 'activity_time_value' | 'road_travel' | 'gear_asset' | 'merch_pod' | 'marketing_growth' | 'fan_crm' | 'publishing_rights' | 'collaboration_splits' | 'licensing_sync' | 'royalty_revenue' | 'legal_compliance' | 'creative_production' | 'opportunity' | 'education_curriculum' | 'security_trust' | 'boardroom_meta';
...
export interface HarnessEvidenceRef {
    id: string;
    type: 'audio' | 'video' | 'image' | 'document' | 'url' | 'screenshot' | 'identifier' | 'metadata' | 'statement' | 'hash';
    label: string;
    value?: string;
    url?: string;
    hash?: string;
...
export type HarnessCostType = 'cash_expense' | 'time_value' | 'mileage' | 'asset_depreciation' | 'inventory_cost' | 'service_fee' | 'royalty_obligation' | 'opportunity_cost' | 'legal_protection_cost';
...
export interface BusinessActivityEvent {
    ...
    releaseId?: string;
    tourId?: string;
    category: 'creative_labor' | 'admin_labor' | 'marketing_labor' | 'travel_labor' | 'legal_labor' | 'learning' | 'unallocated';
    startedAt: string;
    endedAt?: string;
...
export declare function createHarnessRun<TOutput>(params: Omit<HarnessRun<TOutput>, 'runId' | 'createdAt' | 'schemaVersion'> & {
    runId?: string;
    createdAt?: string;
    schemaVersion?: number;
}): HarnessRun<TOutput>;
