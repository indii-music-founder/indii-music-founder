...
import { FileText, Mail, Calendar, MapPin, Users, ChevronDown, Cpu, Globe } from 'lucide-react';
...
export const TourBookTab: React.FC<TourBookTabProps> = ({ itinerary, onUpdateStop }) => {
    ...
    const [activeTab, setActiveTab] = useState<'day-sheets' | 'tech-rider' | 'international'>('day-sheets');
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden p-6">
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-400">{itinerary?.stops?.length || 0} stops • {itinerary?.tourName || 'Unnamed Tour'}</p>
            </div>

            <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as typeof activeTab)} className="flex flex-col flex-1">
                <TabsList className="grid w-full grid-cols-3 mb-6">
                    <TabsTrigger value="day-sheets" className="flex items-center gap-2">
                        <FileText size={14} />
                        <span className="hidden sm:inline">Day Sheets</span>
                        <span className="sm:hidden text-xs">Sheets</span>
                    </TabsTrigger>
                    <TabsTrigger value="tech-rider" className="flex items-center gap-2">
                        <Cpu size={14} />
                        <span className="hidden sm:inline">Tech Rider</span>
                        <span className="sm:hidden text-xs">Tech</span>
                    </TabsTrigger>
                    <TabsTrigger value="international" className="flex items-center gap-2">
                        <Globe size={14} />
                        <span className="hidden sm:inline">International</span>
                        <span className="sm:hidden text-xs">Intl</span>
                    </TabsTrigger>
                </TabsList>

                <TabsContent value="day-sheets" className="flex-1 overflow-y-auto">
                ...
                <TabsContent value="tech-rider" className="flex-1 overflow-y-auto">
                    <TechnicalRiderGenerator />
                </TabsContent>

                <TabsContent value="international" className="flex-1 overflow-y-auto">
                    <VisaChecklist />
                </TabsContent>
            </Tabs>
            {/* Day Sheet Modal */}
                                ...
                                >
                                    <CardContent className="pt-0 border-t border-gray-800 space-y-4">
                                        ...
                                        <div className="flex gap-2 pt-2">
                                            ...
                                            <Button
                                                onClick={() => onSendAdvanceEmail(stop)}
