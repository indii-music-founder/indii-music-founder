...
export const DistributorInfoCard = ({ distributorName }: DistributorInfoCardProps) => {
    ...
    return (
        <div className={`mt-4 bg-linear-to-br from-blue-500/10 via-blue-500/5 to-transparent border ${getColorForModule('distribution').border}/20 rounded-xl p-4 animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-lg`}>
            <div className="flex items-start gap-4 mb-5 border-b border-white/10 pb-4">
                <div className={`p-2.5 ${getColorForModule('distribution').bg} rounded-lg border ${getColorForModule('distribution').border}/30`}>
                    <BookOpen size={20} className={getColorForModule('distribution').text} />
                </div>
                <div>
                    <p className={`text-[10px] ${getColorForModule('distribution').text} font-bold uppercase tracking-[0.15em] mb-1`}>{distro.name} Protocol</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
                    <div className="flex items-center gap-2 mb-2">
                        <Image size={14} className={getColorForModule('distribution').text} />
                ...
                <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
                    <div className="flex items-center gap-2 mb-2">
                        <Music size={14} className={getColorForModule('distribution').text} />
                ...
                <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
                    <div className="flex items-center gap-2 mb-2">
                        <Clock size={14} className={getColorForModule('distribution').text} />
                ...
                <div className="bg-white/[0.03] rounded-xl p-3 border border-white/5">
                    <div className="flex items-center gap-2 mb-2">
                        <DollarSign size={14} className={getColorForModule('distribution').text} />
            ...
            <div className="bg-white/[0.03] rounded-xl p-3 mb-5 border border-white/5">
                <div className="flex items-center gap-2 mb-3">
                    <FileCheck size={14} className={getColorForModule('distribution').text} />
                ...
                <div className="flex flex-wrap gap-1.5">
                    {distro.metadata.requiredFields.slice(0, 8).map((field, idx) => (
                        <span key={idx} className={`text-[9px] uppercase font-bold tracking-wider ${getColorForModule('distribution').bg} ${getColorForModule('distribution').text} px-2 py-1 rounded-md border ${getColorForModule('distribution').border}/20`}>
                            {field}
                        </span>
                    ))}
                </div>
            ...
            {distro.proTips.length > 0 && (
                <div className="border-t border-white/10 pt-4">
                    <p className={`text-[10px] ${getColorForModule('distribution').text} font-bold uppercase tracking-[0.15em] mb-2.5`}>Distributor Insights</p>
                    <ul className="space-y-2">
                        {distro.proTips.slice(0, 3).map((tip, idx) => (
                            <li key={idx} className="text-xs text-gray-400/90 flex items-start gap-3 leading-relaxed">
                                <span className={`${getColorForModule('distribution').text} font-bold`}>→</span>
                                {tip}
                            </li>
                        ))}
                    </ul>
