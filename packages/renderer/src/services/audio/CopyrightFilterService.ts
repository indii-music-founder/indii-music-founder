...
export class CopyrightFilterService {
    ...
    private async queryRegistry(fingerprint: string, _filename: string): Promise<CopyrightReport> {
        void fingerprint;
        throw new Error('Copyright fingerprint registry access is backend-only. Configure a secured Firebase ACRCloud gateway before generating clearance reports.');
    }
