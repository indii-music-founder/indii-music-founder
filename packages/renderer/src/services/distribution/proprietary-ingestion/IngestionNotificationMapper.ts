...
/**
 * IngestionNotification Mapper
 * Transforms internal GoldenMetadata into strict Ingestion IngestionNotification 4.3 Objects
 */
export class IngestionNotificationMapper {
      static mapMetadataToIngestionNotification(
          ...
          // 1. Build Header
          const messageHeader: IngestionMessageHeader = {
    ...
    private static buildMainRelease(
        ...
        if (metadata.aiGeneratedContent) {
                      const disclosureType = IngestionNotificationMapper.classifyAIDisclosure(metadata.aiGeneratedContent);
        ...
        if (!metadata.publisher || metadata.publisher === 'Self' || metadata.publisher === 'self') {
            release.rightsControllers = [{
                               entityName: metadata.labelName || metadata.artistName,
    ...
    /**
     * Generates a unique reference for the IngestionNotification message
     */
