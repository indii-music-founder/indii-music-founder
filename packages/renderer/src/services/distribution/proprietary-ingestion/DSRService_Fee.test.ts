import { describe, it, expect } from 'vitest';
import { earningsReportService } from './EarningsReportService';
import { DISTRIBUTORS } from '@/core/config/distributors';
import type { EarningsReportReport } from './types/dsr';
import type { ExtendedGoldenMetadata } from '@/services/metadata/types';

describe('EarningsReportService Fee Calculation', () => {
    it('should apply distributor fee for Symphonic (15% fee)', async () => {
        // Symphonic has 85% payout, so fee is 15%.
        // Party ID: PASystemIdentityA2011030901S
        const symphonicSystemIdentifier = DISTRIBUTORS.symphonic!.systemIdentifier;

        const mockReport: EarningsReportReport = {
            reportId: 'RPT-SYM-001',
            senderId: symphonicSystemIdentifier,
            recipientId: 'PASystemIdentityA',
            reportingPeriod: { startDate: '2025-01-01', endDate: '2025-01-31' },
            reportCreatedDateTime: '2025-02-01T12:00:00Z',
            currencyCode: 'USD',
            summary: {
                totalUsageCount: 1,
                totalRevenue: 100,
                currencyCode: 'USD'
            },
            transactions: [
                {
                    transactionId: 'TX-1',
                    resourceId: { isrc: 'USSYM1234567' },
                    usageType: 'Download',
                    usageCount: 1,
                    revenueAmount: 100,
                    currencyCode: 'USD',
                    territoryCode: 'US'
                }
            ]
        };

        const mockMetadata: ExtendedGoldenMetadata = {
            title: 'Symphonic Track',
            artist: 'Test Artist',
            isrc: 'USSYM1234567',
            upc: '1234567890123',
            releaseDate: '2024-01-01',
            genre: 'Pop',
            releaseType: 'Single',
            territories: ['US'],
            splits: [
                { email: 'artist@test.com', percentage: 100, role: 'Main Artist', legalName: 'Artist' }
            ],
            tracks: [],
            copyrightYear: '2024',
            copyrightOwner: 'Test Label'
        } as unknown as ExtendedGoldenMetadata;

        const catalog = new Map<string, ExtendedGoldenMetadata>();
        catalog.set('USSYM1234567', mockMetadata);

        const result = await earningsReportService.processReport(mockReport, catalog);

        const royalty = result.royalties[0];

        // Gross Revenue = 100
        expect(royalty!.grossRevenue).toBe(100);

        // Fee should be 15% of 100 = 15
        expect(royalty!.distributorFees).toBe(15);

        // Net Revenue should be 100 - 15 = 85
        expect(royalty!.netRevenue).toBe(85);
    });

    it('should apply 0 fee for DistroKid (100% payout)', async () => {
        // DistroKid has 100% payout, so fee is 0%.
        const distrokidSystemIdentifier = DISTRIBUTORS.distrokid!.systemIdentifier;

        const mockReport: EarningsReportReport = {
            reportId: 'RPT-DK-001',
            senderId: distrokidSystemIdentifier,
            recipientId: 'PASystemIdentityA',
            reportingPeriod: { startDate: '2025-01-01', endDate: '2025-01-31' },
            reportCreatedDateTime: '2025-02-01T12:00:00Z',
            currencyCode: 'USD',
            summary: {
                totalUsageCount: 1,
                totalRevenue: 100,
                currencyCode: 'USD'
            },
            transactions: [
                {
                    transactionId: 'TX-1',
                    resourceId: { isrc: 'USDK1234567' },
                    usageType: 'Download',
                    usageCount: 1,
                    revenueAmount: 100,
                    currencyCode: 'USD',
                    territoryCode: 'US'
                }
            ]
        };

        const mockMetadata: ExtendedGoldenMetadata = {
            title: 'DistroKid Track',
            artist: 'Test Artist',
            isrc: 'USDK1234567',
            splits: [{ email: 'artist@test.com', percentage: 100, role: 'Main Artist', legalName: 'Artist' }]
        } as unknown as ExtendedGoldenMetadata;

        const catalog = new Map<string, ExtendedGoldenMetadata>();
        catalog.set('USDK1234567', mockMetadata);

        const result = await earningsReportService.processReport(mockReport, catalog);
        const royalty = result.royalties[0];

        expect(royalty!.distributorFees).toBe(0);
        expect(royalty!.netRevenue).toBe(100);
    });
});
