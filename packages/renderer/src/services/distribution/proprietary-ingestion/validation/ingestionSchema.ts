...
/**
 * Ingestion Zod Schemas
 * Comprehensive validation for IngestionNotification 4.3 metadata
 */

// --- Common Primitives ---

export const SystemIdentitySchema = z.object({
    systemIdentifier: z.string().regex(/^PASystemIdentityA[a-zA-Z0-9]{10}$/, 'Invalid SystemIdentity format (Must be PASystemIdentityA + 10 characters)'),
    entityName: z.string().min(1, 'Party name is required')
});
...
// --- IngestionNotification Specific Components ---
...
export const IngestionNotificationMessageSchema = z.object({
    messageSchemaVersionId: z.literal('4.3'),
    messageHeader: z.object({
        messageId: z.string().uuid().or(z.string().min(1)),
        messageSender: SystemIdentitySchema,
        messageRecipient: SystemIdentitySchema,
...
/**
 * Validates a Ingestion IngestionNotification object against the schema.
 * Returns a ValidationResult compatible with indii internal types.
 */
export function validateIngestionNotification(data: unknown) {
    const result = IngestionNotificationMessageSchema.safeParse(data);
