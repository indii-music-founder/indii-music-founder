...
export class IngestionValidator {
    ...
    validateXML(xml: string, schemaVersion: string = '4.3'): { valid: boolean; errors: string[] } {
        ...
        if (!xml.includes('NewReleaseMessage') && !xml.includes('PurgeReleaseMessage')) {
            errors.push('Missing valid Ingestion root element (NewReleaseMessage)');
        }

        // Check for Ingestion Namespace
        if (!xml.includes('http://ddex.net/xml/ern')) {
            errors.push('Missing Ingestion namespace declaration');
        }
...
export const ingestionValidator = new IngestionValidator();
