...
export class IntelligenceResponseCache {
        ...
        try {
            ...
            if (navigator.storage?.estimate) {
                ...
                if (remaining < 50 * 1024 * 1024) { // < 50MB remaining
                    logger.warn('[IntelligenceResponseCache] IndexedDB quota low (<50MB remaining) — skipping cache write');
                    return;
                }
            }
    ...
    private addToL1(key: string, entry: CachedResponse) {
        if (this.l1Cache.size >= IntelligenceResponseCache.MAX_L1_SIZE) {
...
export const aiCache = new IntelligenceResponseCache();
