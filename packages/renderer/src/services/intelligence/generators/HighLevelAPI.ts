/**
 * HighLevelAPI — Extracted high-level convenience methods from FirebaseIntelligenceService.
 *
...
export async function generateText(
    ...
    if (!result?.response) {
        throw new AppException(AppErrorCode.INTERNAL_ERROR, 'Intelligence Service returned an invalid response structure');
    }
...
export async function generateStructuredData<T>(
    ...
    if (!result?.response) {
        throw new AppException(AppErrorCode.INTERNAL_ERROR, 'Intelligence Service returned an invalid response structure for structured data');
    }

    const text = typeof result.response.text === 'function' ? result.response.text() : '';
    if (!text) {
        throw new AppException(AppErrorCode.INTERNAL_ERROR, 'Intelligence Service returned an empty response for structured data');
    }
...
/**
 * Analyze a file URI generated by the GeminiFileService (supports files up to 2GB).
 */
export async function analyzeFileURI(
    ctx: IntelligenceContext,
    fileUri: string,
    mimeType: string,
    ...
    const content: Content[] = [{
        ...
        parts: [
            { text: prompt },
            { fileData: { mimeType, fileUri } } as unknown as Part // Cast to bypass Firebase Autonomous strict part typing if needed
        ]
    }];
