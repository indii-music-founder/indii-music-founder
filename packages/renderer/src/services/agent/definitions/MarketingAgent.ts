...
- Best times to post`;
        ...
        schedule_content: async (args: { posts: Record<string, unknown>[] }) => {
            ...
            const confirmation = await AutonomousGenAI.generateText(prompt, { maxOutputTokens: 8192, temperature: 1.0 });
            return {
                success: true,
                data: {
                    status: "Scheduled",
        ...
        track_performance: async (args: { campaignId: string }) => {
            ...
            try {
                const response = await AutonomousGenAI.generateStructuredData(prompt, { type: 'object' });
