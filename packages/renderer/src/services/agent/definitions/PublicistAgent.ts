...
<body>
  <div class="header">
    <h1>${args.title}</h1>
    <p style="color: #666; font-size: 14px;">Generated by indii Publicist • ${new Date().toLocaleDateString('en-US')}</p>
  </div>
  <div class="content">
        ...
        functionDeclarations: [{
            ...
            parameters: {
                ...
                properties: {
                    ...
                    featuredTracks: { type: "ARRAY", items: { type: "STRING" }, description: "List of Spotify/Apple track IDs or indii audio URLs." },
...
Return only valid JSON, no markdown fences.`;
    ...
    .withTool({
        functionDeclarations: [{
            ...
            description: "Draft a professional press release for a new music release, tour, or announcement.",
            parameters: {
                type: "OBJECT",
                properties: {
                    artist_name: { type: "STRING", description: "The artist's name." },
                    announcement_type: { type: "STRING", description: "e.g., Album Release, Tour, Signing" },
                    release_date: { type: "STRING", description: "Release date in human-readable form." },
                    key_details: { type: "ARRAY", items: { type: "STRING" }, description: "List of key facts or details." },
                    quotes: { type: "ARRAY", items: { type: "STRING" }, description: "Optional quotes from the artist or management." }
                },
                required: ["artist_name", "announcement_type", "key_details"]
            }
        }]
    }, async (args: { artist_name: string; announcement_type: string; release_date?: string; key_details: string[]; quotes?: string[] }) => {
        /**
         * Draft a professional press release.
         */
        const prompt = `You are an expert music publicist. Draft a professional press release for ${args.artist_name}.
Announcement Type: ${args.announcement_type}
${args.release_date ? `Release/Event Date: ${args.release_date}` : ''}
Key Details:
${args.key_details.map(d => `- ${d}`).join('\n')}
${args.quotes ? `Quotes to include:\n${args.quotes.map(q => `- "${q}"`).join('\n')}` : ''}
...
Format as a standard, ready-to-publish press release with a catchy headline, dateline, introduction, body paragraphs, and a boilerplate "About the Artist" section at the end.`;
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return { success: false, error: `Failed to draft press release: ${message}` };
        }
    })
    .withTool({
        functionDeclarations: [{
            ...
            parameters: {
                type: "OBJECT",
                properties: {
                    genre: { type: "STRING", description: "Musical genre (e.g., Hip-Hop, Indie Rock, Electronic)." },
                    region: { type: "STRING", description: "Target region or 'Global'." },
                    contact_type: { type: "STRING", description: "e.g., Blog, Playlist, Magazine, Radio." }
                },
                required: ["genre", "contact_type"]
            }
        }]
