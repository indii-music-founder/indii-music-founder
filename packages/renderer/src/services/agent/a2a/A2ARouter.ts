...
class A2ARouter {
  ...
  private async dispatchStreamInit(
    ...
    if (!localCtx?.runAgent && !localCtx?.streamAgent) {
      throw new Error('No runAgent or streamAgent available');
    }

    const requestId = crypto.randomUUID();
    // Prefer true incremental streaming when a streamAgent is available; otherwise
    // fall back to the batch generator (single final envelope). Chunks are encrypted
    // back to the actual caller (senderId), never a hardcoded id.
    const generator = localCtx.streamAgent
    ...
    logger.info(`[A2ARouter] Stream initialized for ${targetAgentId}: ${requestId} (${localCtx.streamAgent ? 'streaming' : 'batch'})`);
    return { requestId };
  }
