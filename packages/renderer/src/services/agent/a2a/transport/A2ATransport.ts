...
export interface RouterCallContext {
  ...
  traceId?: string;
  /**
   * Streaming runner. When present, the router yields incremental (coalesced) deltas
   * instead of one batch envelope. Forwards each token chunk via onToken.
   */
  streamAgent?: (agentId: string, task: string, onToken: (chunk: string) => void) => Promise<{ text: string }>;
}
