 ...
 *   4. The verificationHash for each entry is SHA-256("{name}|{AGREEMENT_VERSION}|{joinedAt}")
 ...
 *     .digest('hex');
 *   // hash must equal verificationHash in this file for the record to be valid.
 */
...
export const AGREEMENT_TERMS = {
  ...
  // Founders provide their own API keys (e.g., via settings) and are
  // responsible for their own consumption directly with providers.
...
export interface FounderRecord {
  /** Permanent seat number 1-11. Seat #1 is reserved for the builder. */
  seat: number;
 ...
 * THE FOUNDERS
 *
 * Seats 1-11. Seat #1 is the builder's reserved Founder seat; seats #2-#11
