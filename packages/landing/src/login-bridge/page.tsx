...
export default function LoginBridge() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setError('Founder preview sign-in is not configured in this environment.');
            setStatus('ready');
            return;
        }

        const unsubscribe = onAuthStateChanged(authRef.current, () => {
            setStatus('ready');
        });
    ...
    const redirectToApp = async ({ code, idToken, accessToken }: { idToken?: string; accessToken?: string | null; code?: string }) => {
        setStatus('success');
        setError(null);
        try {
            const params = new URLSearchParams();
            params.append('source', 'founder');
            if (code) {
            ...
            await trackFounderFunnelEvent('founder_auth_completed', {
                variant: 'bridge',
                handoff: code ? 'desktop_code' : 'deep_link',
            });
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            if (!authRef.current) throw new Error('Founder preview sign-in is not configured in this environment.');
            ...
            if (handoffCode) {
                await redirectToApp({ code: handoffCode });
            } else {
                await redirectToApp({ idToken: credential.idToken, accessToken: credential.accessToken });
            }
        } catch (err: unknown) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white font-sans">
            <div className="p-8 border border-neutral-800 rounded-xl bg-neutral-900/50 text-center max-w-md w-full">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold mb-2">Founder Preview</h1>
                    <p className="text-neutral-400 text-sm">Sign in to enter the guided walkthrough and continue into the app</p>
                </div>
                ...
                {status === 'ready' && (
                    <button
                        ...
                        disabled={!authRef.current}
                        className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white text-black font-semibold rounded-lg hover:bg-gray-100 transition-colors disabled:cursor-not-allowed disabled:bg-white/30 disabled:text-black/60"
                    >
                ...
                <div className="mt-6 pt-6 border-t border-neutral-800"><p className="text-neutral-500 text-xs">This page authenticates you with Google and returns you to the founder preview in the app.</p></div>
