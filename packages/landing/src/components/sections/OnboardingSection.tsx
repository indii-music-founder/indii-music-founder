...
export default function OnboardingSection() {
  return (
    <section data-system-section="onboarding" className="relative z-20 w-full overflow-hidden border-t border-amber-400/20 bg-gradient-to-b from-[#0F0C09] via-[#120E0A] to-[#0D0A08]">
      {/* Ambient background glow */}
      <div className="pointer-events-none absolute right-0 top-1/2 h-[550px] w-[550px] -translate-y-1/2 rounded-full bg-[#00BCD4]/[0.14] blur-[150px]" />
      <div className="pointer-events-none absolute left-0 top-1/3 h-[500px] w-[500px] rounded-full bg-[#FFB800]/[0.12] blur-[140px]" />
      ...
      <div className="mx-auto grid max-w-[1500px] gap-16 px-5 py-28 md:px-10 md:py-40 lg:grid-cols-[1.05fr_0.95fr] lg:gap-24">
        ...
        >
          <div className="mb-6 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-[#FFB800] [text-shadow:0_0_12px_rgba(255,184,0,0.6)]">
          ...
          <h2 className="max-w-4xl text-5xl font-black leading-[0.93] tracking-[-0.055em] text-white md:text-7xl lg:text-[6.4rem]">
            ...
            <span className="block text-[#FFB800] [text-shadow:0_0_35px_rgba(255,184,0,0.65)]">not an empty dashboard.</span>
          </h2>
        ...
        <motion.div
          ...
          className="card-shade-gold relative overflow-hidden rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,0.85)] backdrop-blur-2xl"
        >
          <div className="absolute inset-x-0 top-0 h-[1px] specular-line-gold" />
          <div className="flex items-center justify-between border-b border-amber-500/20 bg-[#100D09]/90 px-6 py-5 font-mono text-[9px] uppercase tracking-[0.2em] text-white/50">
            <span className="font-bold text-amber-200">Catalog intake / founder</span>
            <span className="rounded-full bg-amber-400/20 px-3 py-1 font-bold text-amber-300 border border-amber-400/40 shadow-[0_0_10px_rgba(255,184,0,0.25)]">Guided</span>
          </div>
          {intakeItems.map((item, index) => (
            <div
              key={item.text}
              style={{ borderLeft: `3px solid ${item.hex}` }}
              className="group flex items-center gap-5 border-b border-white/8 px-6 py-5 transition-all hover:bg-white/[0.04]"
            >
              ...
              <div
                style={{ backgroundColor: `${item.hex}22`, boxShadow: `0 0 12px ${item.hex}40`, border: `1px solid ${item.hex}60` }}
          ...
          <div className="bg-[#100D09]/80 px-6 py-7 text-sm leading-relaxed text-white/65">
