...
export default function WaitlistSection({
  ...
  return (
    <section
      ...
      className="relative z-20 scroll-mt-[104px] border-y border-amber-400/30 bg-[#14100C] shadow-[0_0_50px_rgba(255,184,0,0.12)]"
      aria-labelledby="waitlist-title"
    >
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,184,0,0.09),transparent_65%)]" />
      <div className="relative mx-auto grid max-w-[1500px] gap-8 px-5 py-12 md:grid-cols-[0.48fr_1fr_0.36fr] md:items-center md:px-10 md:py-14">
        ...
        <div>
          ...
          {preferenceMode ? (
            ...
            <div className="mt-3 max-w-2xl space-y-2 text-sm leading-relaxed text-white/60 md:text-base">
              <p>
          ...
          <form onSubmit={onSubmit} className="flex w-full flex-col gap-3 md:items-end">
            <div className="flex w-full flex-col gap-3 sm:flex-row md:justify-end">
              ...
              <input
                ...
                className="w-full rounded-lg border border-amber-400/30 bg-black/60 px-4 py-2.5 text-sm text-white placeholder-white/40 outline-none backdrop-blur-md transition-all focus:border-amber-400 focus:bg-black/80 focus:shadow-[0_0_15px_rgba(255,184,0,0.3)] sm:max-w-[240px]"
              />
              <button
                type="submit"
                disabled={status === 'loading' || status === 'success'}
                className="group inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-gradient-to-r from-[#FFD700] via-[#FFB800] to-[#CCA000] px-6 py-2.5 text-sm font-bold text-black shadow-[0_0_22px_rgba(255,184,0,0.35)] transition-all hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(255,184,0,0.55)] disabled:opacity-50"
              >
                {status === 'loading'
                  ? 'Verifying...'
                  : status === 'sent'
