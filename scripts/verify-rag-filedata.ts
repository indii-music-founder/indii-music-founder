...
if (!process.env.VITE_FUNCTIONS_URL) {
    process.env.VITE_FUNCTIONS_URL = 'https://us-central1-YOUR_FIREBASE_PROJECT_ID.cloudfunctions.net';
    console.log("⚠️ VITE_FUNCTIONS_URL missing, using live: https://us-central1-YOUR_FIREBASE_PROJECT_ID.cloudfunctions.net");
}
...
async function runTest() {
    ...
    try {
        ...
        console.log(`   👉 Using NATIVE RAG (File ID Only) with gemini-3.5-flash`);
        ...
        // Using approved gemini-3.5-flash as the primary RAG target
        console.log(`   👉 Using NATIVE RAG with gemini-3.5-flash (Temperature 0.0)`);
        const result = await service.query(file.name, query, undefined, 'gemini-3.5-flash');
        console.log("   ✅ Query successful with gemini-3.5-flash");
