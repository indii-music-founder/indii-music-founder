...
canonical DDEX standards registry and supplies tested cell-level TSV escaping
and record-cell splitting primitives. The serializer consults the registry so
the adapter does not claim that RDR-RCC is XML or confuse RDR Part 4 with an
ECM Part 4.
...
are legally/business-significant choices. These helpers do not create complete
messages, derive values from indii entity IDs, change canonical rights claims,
decide rights, sign, persist, upload, or transmit anything. Parsed cells remain
wire values; callers must preserve source evidence and provenance and use an
explicit, reviewed field mapping before creating any canonical fact.
