...
indii operates under three primary third-party agreements:
...
- **Merchant:** New Detroit Music LLC (dba indii)
...
**Firestore:** BigQuery exports available (daily snapshots to GCS bucket `gs://indii-training-data/exports/`). Data is portable.
...
|------|-----------|--------|
| **Ownership of Generated Code** | User (indii) owns all code generated by Claude API | No conflict; all code owned by New Detroit Music LLC |
...
| Term | Provision | Impact |
|------|-----------|--------|
| **Ownership of Generated Code** | User (indii) owns all code generated via Gemini API | No conflict; all code owned by New Detroit Music LLC |
...
indii does not store payment card data directly. All payment processing is handled by Stripe (PCI-DSS Level 1 compliant). No PCI audit required for indii.
...
**Post-Acquisition:** Acquirer assumes all tax liability for indii operations as of closing date. William's personal 2025 tax liability ends at closing.
