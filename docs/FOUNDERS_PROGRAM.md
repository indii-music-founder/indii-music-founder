# indii-Alpha-Electron: Founders Program Documentation
...
*   **1.2 Exclusion of Equity:** This fee does not constitute an investment, purchase of equity, stock, or ownership stake in indii, its parent entities, or its intellectual property.
...
3.  **Code-Level Legal Disclaimer:** The `founders.ts` file must contain the following block comment at the top:
    ```typescript
    /**
     * @notice indii-Alpha-Electron Founders Program
     ...
     * rights over the indii-Alpha-Electron codebase, application, or business entity.
     */
    ```
...
*   **3.2 Firebase App Check:** Implement Firebase App Check (via reCAPTCHA Enterprise or device-level attestation) to ensure only the compiled `indii-Alpha-Electron` binary can communicate with the backend, preventing unauthorized postman/curl queries.
