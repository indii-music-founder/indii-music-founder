...
flowchart TD
    ...
    subgraph "Settings Modal (Cmd+, / Bottom Rail)"
...
- **Ambition dial:** user preference (configured in Settings modal accessed via Sidebar Bottom Rail Dock or `Cmd+,`) → `ContextPipeline` → `AgentContext.ambitionLevel` → contract wording (0/2/4 offered ideas). No silent auto-tuning; growth is consent-based (deferred v1.5 roadmap item, not built now).
