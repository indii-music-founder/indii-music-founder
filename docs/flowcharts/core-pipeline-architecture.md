...
This document maps the newly implemented four-phase core agent development pipeline (`/start` -> `/proceed` -> `/middle` -> `/end`) and its integrations with sub-commands and tools.
...
1. **User Action to Routing (`A -> B`):** The agent identifies the context. If it's a new feature, it runs `/start`. If resuming, it runs `/proceed`. If building, `/middle`. If wrapping up, `/end`.
2. **Start Phase (`C`):** 
