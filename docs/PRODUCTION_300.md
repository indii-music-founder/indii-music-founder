...
- [x] **219. Proprietary Ingestion IP IngestionNotification XSD Schema Validation:** Run the generated IngestionNotification XML through an XSD validator before SFTP upload to catch schema violations before distributors reject the delivery.
...
- [x] **233. YouTube Content ID Real Delivery Parameters:** The Content ID opt-in toggle in the distribution UI stores a flag but never sends the actual content policy to YouTube CMS during delivery. Wire this into the Proprietary Ingestion IP metadata blob.
...
- [x] **253. Firestore Security Rules Unit Tests:** `src/test/security/firestore.rules.test.ts` created — 10 test suites, 32 assertions covering: unauthenticated denial, owner-only access, cross-user denial, anonymous blocked from commercial ops (ingestionReleases, licenses, orgs), org/tax-profile delete hard-blocked, ISRC immutability, rate-limit docId regex, finance owner-only, deny-all unlisted collections. Separate `vitest.rules.config.ts` (node env) + `npm run test:rules` script added.
...
- [x] **263. Firestore Composite Index Audit:** `firestore.indexes.json` — added composite indexes for `split_escrows`, `compositions`, `licensing_deals`, `publishing_registrations`, `ingestionReleases.releaseDate`, `deployments`, and `fraud_alerts`. Now covers all collections introduced in PRODUCTION_200+.
...
- [x] **279. Distribution Delivery E2E Test:** `e2e/distribution-workflow.spec.ts` extended with `Distribution Delivery Pipeline` suite — mocks `initiateDelivery`, `getDeliveryStatus`, `validateProprietary Ingestion IP` CFs; tests delivery ID returned, QC warnings surfaced, status polling response shape.
...
- [x] **284. Load Testing Baseline:** `scripts/load-test.js` — k6 script. 4-stage ramp: 10 → 50 → 100 VUs → 0 over 4 minutes. Tests `generateContent` (P95 < 3s), `createRelease` (P95 < 1s), `getSubscriptionStatus` (P95 < 500ms). Error rate threshold < 1%. Custom `Trend`/`Rate`/`Counter` metrics. Outputs `scripts/load-test-results.json` + console summary. Run: `k6 run scripts/load-test.js -e BASE_URL=<CF_URL> -e AUTH_TOKEN=<token>`.
...
- [x] **297. Secrets Migration to Google Secret Manager:** Migration procedure documented: (1) `gcloud secrets create <NAME> --data-file=.env` for each secret; (2) Cloud Functions Gen 2 reference secrets via `defineSecret()` from `firebase-functions/params`; (3) CI pipeline uses Workload Identity Federation instead of service account JSON key; (4) `.env` files retained only for local dev, never committed. See `docs/API_CREDENTIALS_POLICY.md` for the full policy. Implementation requires GCP project access — infrastructure ticket filed.
