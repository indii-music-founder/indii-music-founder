...
- Phase A1: ☑ A1.2/A1.3/A1.4 shipped; ☑ A1.6 RESOLVED (founder signed off degraded geometry mode, 2026-08-29 — FacePipeline runs @mediapipe FaceLandmarker geometry, embeddingMode 'geometry', scale-invariant geometryFitSimilarity; honest: geometry-fit, NOT identity). ⛔ BLOCKED: A1.1 (@vladmandic/human identity backend, not installed), A1.5 (real-pair threshold calibration — needs founder's real likeness + one generated image), A1.7 (panel smoke). fuse_likeness surfaces a specific 'not configured' error until then; FACE_LANDMARKER_MODEL_PATH must be wired to a bundled face_landmarker.task. See `.agent/FOUNDER_BLOCKERS.md`.
...
- Phase C2: ☑ C2.1+C2.2 shipped (4 canvas agent tools: canvas_open_image/add_layer/set_adjustments/export, registered on Director+CreativeAgent+registry+catalog+tests; applyTransformPatch Fabric object:modified sync, idempotent). ☑ C2.3 split-subject: founder APPROVED @imgly (AGPL-3.0 code + IMG.LY-proprietary weights self-hosted via `scripts/vendor-imgly-weights.mjs`; `SplitSubjectService` + "Split subject" editor op shipped). C2.4 real smoke pending founder.
