...
- [ ] **B1.1** FontLibrary upload/parse/register/persist tests green (jsdom + test-setup mocks).
- [ ] **B1.2** Golden vector snapshot tests green and stable across runs.
- [ ] **B1.3** .woff2 and non-Latin input rejected with specific, user-actionable errors.
- [ ] **B1.4** `rasterizeVectorText` produces transparent-background PNG at requested scale (canvas-mocked per test setup, verified dimensionally).
...
- [ ] **C1.1** `createDocFromImage` + slice action unit tests (state shape, immutability, reorder bounds).
- [ ] **C1.2** AdjustmentStack → Fabric filter mapping unit tests: each field maps to exactly one filter instance; neutral stack applies zero filters.
