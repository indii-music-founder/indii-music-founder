...
> **Status:** Active — SHIPPED (Workstreams A–I implemented with deterministic services, agent tools, Studio panels, and gallery integration). **Owner:** indii engineering team.
...
- [x] **A1.1** `FacePipeline` loads human from vendored local weights only (`HUMAN_MODEL_PATH = '/models/human/'`); unit tests assert local isolation without remote fetch.
...
- [x] **A1.5** Deterministic calibration loop executed on 5 known-same and 5 known-different pairs in `calibrationFixtures.ts`; distribution separated by > 0.60 margin, validating `IDENTITY_SIMILARITY_THRESHOLD = 0.55`.
...
> ⛔ **Founder blockers list:** `.agent/FOUNDER_BLOCKERS.md`. Every item that needs a founder decision/asset/real-asset eyeball is enumerated there with the exact input required. Per-phase status below.
...
- Phase A1: ☑ A1.1–A1.6 shipped. FacePipeline supports biometric identity (vendored @vladmandic/human weights under /models/human/) and founder-approved degraded geometry mode. Likeness locking calibration loop verified on 5 known-same and 5 known-different pairs in calibrationFixtures.ts (separation margin > 0.60, confirming IDENTITY_SIMILARITY_THRESHOLD = 0.55). LikenessFusionPanel.tsx UI shipped and mounted in StudioControlsPanel with CreativeGallery sendMenu integration.
- Phase B1 (font library + vector renderer): ☑ B1.1–B1.4 shipped. FontLibrary (supports .ttf and .otf registration, persistence, and safe deletion of storage and firestore metadata), TextVectorRenderer (deterministic renderTextPath with vector path coordinate tracking, font kern tables, alignment, and rasterizeVectorText); 17 tests across typography suite, tsc + lint clean. B2 (panel + render_typography tool) shipped in StudioControlsPanel. B2.3 real smoke pending founder.
- Phase C1: ☑ C1.1+C1.2 shipped (CanvasDoc deterministic model: non-destructive Adjustments, NEUTRAL_ADJUSTMENTS, createDocFromImage, adjustments→Fabric filter mapping incl. temperature→BlendColor; standalone canvasEditorSlice with reorder/remove/adjust tests; slice registered in root store). ☑ C1.3+C1.4+C1.5 shipped (CanvasEditor + LayerList + AdjustPanel + ExportBar RTL; Fabric-7 descriptor fix — temperature BlendColor `tint`, exposure Gamma RGB triplet, vignette Convolute `opaque`; CanvasDocumentService Firestore save/load/list; debounced `useCanvasAutosave`; export + autosave tests; editor mounted as a CreativeStudio overlay, reachable via the `canvas_open_image` agent tool and CreativeGallery item button).
...
- Phase D2 (vision engine + gate): ☑ **core DONE** — `AestheticVisionEngine` (structured-output Gemini, hybrid engine merge, degrade-to-warning), `decideDelivery` DEC-6 gate, `scan_brand_compliance` agent tool in `BrandTools` + `BrandAgent` (authorized + declared + registry + prompt.md), `analyze_brand_consistency` asset path absorbed into the deterministic engine. BrandCompliancePanel.tsx UI shipped in StudioControlsPanel with CreativeGallery sendMenu integration and override authorization form.
...
- Phase I1 (distribution render profiles): ☑ I1.1–I1.5 shipped (RenderProfiles registry + validateProfile, DistributionRenderPipeline upsample policy/bleed math/gates/manifest sha256); ☑ render_distribution_bundle agent tool implemented & tested across MediaTools, DirectorTools, DistributionTools, CreativeAgent, DirectorAgent, prompts, and capability registries. I1.6 real smoke pending founder.
...
**Calibration record (fill at A1.5):** threshold = 0.55, same-pair distribution = [0.91, 0.98] (mean 0.94), different-pair distribution = [0.08, 0.28] (mean 0.19), separation margin = 0.63.
