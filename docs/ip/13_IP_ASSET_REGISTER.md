...
| IP-BRAND-010 | `indii Product Skills©` / `indii Product Skills™` & `Studio Skills™` | Proprietary marks for the codified music business intelligence framework, domain playbooks, and procedural execution runtime. Documented in `docs/product/PRODUCT_SKILLS_MANUAL.md`. | Core technology & feature mark. **Needs trademark filing (Classes 009, 042).** |
| IP-BRAND-011 | `The Sonic Director©` / `The Sonic Director™` | Proprietary mark for audio intelligence, spectral car-test EQ analysis, -14 LUFS compliance, and acoustic profiling software. | Audio intelligence technology mark. **Needs trademark filing (Classes 009, 041, 042).** |
| IP-BRAND-012 | `The Boardroom©` / `Boardroom Meta-Harness™` | Proprietary mark for multi-agent autonomous risk arbitration, cross-domain conflict reconciliation, and executive meta-decision governance. | Orchestration product mark. **Needs trademark filing (Classes 009, 042).** |
| IP-BRAND-013 | `Artist Master Directive™` & `Studio DNA Playbook™` | Proprietary marks for the living, user-co-authored operational playbook and dynamic override skill framework. | Living playbook technology mark. **Needs trademark filing (Classes 009, 042).** |
| IP-BRAND-014 | `Living Skill Protocol™` | Proprietary mark for bi-directional real-time reflective knowledge distillation between human artists and autonomous multi-agent swarms. | Swarm intelligence protocol mark. **Needs trademark filing (Classes 009, 042).** |
| IP-BRAND-015 | `Visual DNA Cascading Engine™` / `Visual DNA™` | Proprietary mark for real-time universal color palette and aesthetic rule propagation across autonomous agent prompts, DOM CSS properties, living directives, and UI components. | Visual identity cascading mark. **Needs trademark filing (Classes 009, 042).** |
...
| IP-PROCESS-006 | Hermetic Progressive Disclosure & Compile-Time Playbook Bundling Protocol | Proprietary technical process. Evidence: `docs/product/PRODUCT_SKILLS_MANUAL.md`, `packages/renderer/src/services/agent/skills/ProductSkillRegistry.ts`, and `ContextPipeline.ts`. Method of packaging domain SOPs via build-time static raw imports into client application binaries, dynamically resolving schema constraints, and injecting progressive disclosure layers (L1 discovery ➔ L2 playbook context ➔ L3 deterministic tools) into local LLM contexts without runtime filesystem dependencies. | High-value utility patent candidate. **Verified technically / needs patent counsel review.** |
| IP-PROCESS-007 | Dual-State Dynamic Invocability & Approval-Gated Tool Sandboxing Protocol | Proprietary governance process. Evidence: `agents/conductor/skills/`, `ApprovalGateRegistry.ts`, and `PromptArea.tsx`. Coordination of `disable-model-invocation`, `user-invocable`, and `allowed-tools` parameters with an application-level gate registry that enforces cryptographic artist approval prior to executing irreversible distribution, legal, or financial actions. | High-value utility patent candidate. **Verified technically / needs patent counsel review.** |
| IP-PROCESS-008 | Cross-Domain Business Harness Conflict Reconciliation & Meta-Arbitration Engine | Proprietary multi-agent arbitration process. Evidence: `services/business-harness/BoardroomMetaHarnessService.ts`, `HarnessCatalog.ts`, and `agents/conductor/skills/business_harness_system/SKILL.md`. Automated arbitration engine that captures conflicting findings across distinct domain agents (Marketing, Finance, Legal, Touring) and calculates an integrated risk-weighted meta-decision with blocker thresholds. | High-value utility patent candidate. **Verified technically / needs patent counsel review.** |
| IP-PROCESS-009 | Bi-Directional Co-Authored Dynamic Playbook Inheritance Protocol | Proprietary multi-agent inheritance process. Evidence: `packages/shared/src/schemas/artistMasterDirective.ts`, `packages/renderer/src/services/agent/skills/ArtistDirectiveService.ts`, `packages/renderer/src/services/agent/tools/ArtistDirectiveTools.ts`, and `MasterPlaybookSection.tsx`. Real-time co-authoring between user and AI Conductor with supreme prompt override precedence over base domain playbooks. | High-value utility patent candidate. **Verified technically / needs patent counsel review.** |
| IP-PROCESS-010 | Universal Visual DNA & Dynamic Brand Asset Swarm Cascading Protocol | Proprietary multi-agent visual synchronization process. Evidence: `packages/renderer/src/services/brand/BrandSyncService.ts`, `BrandTools.ts`, `AgentPromptBuilder.ts`, `MasterPlaybookSection.tsx`, and `agents/conductor/skills/brand_manager/SKILL.md`. Real-time 5-tier simultaneous cascading engine for artist brand colors and aesthetic rules across store, master directive, DOM styles, agent prompts, and decoupled UI event buses. | High-value utility patent candidate. **Verified technically / needs patent counsel review.** |

| IP-COPYRIGHT-001 | indii Conductor 27 Domain Specialist Playbook Corpus | Proprietary literary & technical work of authorship. Evidence: `agents/conductor/skills/*/SKILL.md` (27 domain playbooks including `audio_engineering`, `digital_distribution`, `publishing`, `sync_licensing`, `finance_royalties`, `legal_affairs`, etc.). | High-value proprietary know-how. **Needs US Copyright Office Form TX registration.** |
| IP-COPYRIGHT-002 | indii Product Skills Manual & Runtime Architecture Specification | Proprietary technical documentation & architectural diagram. Evidence: `docs/product/PRODUCT_SKILLS_MANUAL.md` and `docs/flowcharts/product-skill-runtime-architecture.md`. | Institutional training & product manual. **Needs US Copyright Office Form TX registration.** |
