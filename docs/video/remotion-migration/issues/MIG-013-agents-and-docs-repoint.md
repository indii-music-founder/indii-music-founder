...
- [x] General editor render is `queue_video_render(projectId, outputName)`; the old
      release-canvas backend is separately and honestly named `queue_release_canvas_render`
- [x] Video Agent can inspect the active project, add/update clips, update keyframes,
      and render through the same live editor model/preview path used by a person
