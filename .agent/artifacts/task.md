...
- [ ] ISSUE-1007 — "Cover Art" mode promises distributor compliance but never verifies delivered file
  - **Status:** 🟡 PARTIAL (verified 2026-07-20) — canonical release artwork is content-addressed, immutable, hash/size/MIME checked, remeasured at desktop staging, and rejected unless it is square RGB JPEG/PNG at least 3000px; Python packaging preserves the measured RGB gate. The server audit now creates a release-attached immutable receipt with the object generation/digest, `dsp-cover-art-baseline.v1`, and explicit generator/model provenance (`not_recorded` rather than invented when absent). Remaining: invoke the server audit from both release-submission paths before delivery-ready state, preserve real generation-provider/model/version evidence at creation, and offer an actionable repair path instead of only failure messaging.
