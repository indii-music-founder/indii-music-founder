...
- [ ] ISSUE-1007 — "Cover Art" mode promises distributor compliance but never verifies delivered file
  - **Status:** 🟡 PARTIAL (verified 2026-07-20) — canonical release artwork is content-addressed, immutable, hash/size/MIME checked, remeasured at desktop staging, and rejected unless it is square RGB JPEG/PNG at least 3000px; Python packaging preserves the measured RGB gate. Both the desktop route and the older publishing wizard now preserve one canonical cover identity; the wizard persists its stable path/hash/provenance instead of a timestamped packaging copy and refuses packaging until the server audit attaches an immutable receipt with the object generation/digest, `dsp-cover-art-baseline.v1`, and explicit generator/model provenance (`not_recorded` rather than invented when absent). Remaining: route the separate desktop path through that same persisted gate, preserve real generation-provider/model/version evidence at creation, and offer an actionable repair path instead of only failure messaging.
