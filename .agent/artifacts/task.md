...
- [x] ISSUE-1083 — Audio profiling callable accepted arbitrary Storage paths and queued an unauthenticated placeholder engine target
  - **Status:** ✅ FIXED (production verified 2026-07-20) — `processAudioIngestion`, the private `engine-dsp` Cloud Run service, `dsp-processing-queue`, OIDC invoker identity, and least-privilege runtime boundary are deployed. Real owner-scoped WAV and FLAC masters queued through the callable, reached the private worker, and persisted hash/generation-bound receipts; cross-owner dispatch remains rejected. Focused Firebase/renderer contracts pass (14 tests), with live task/OIDC/receipt evidence retained in production.
- [x] ISSUE-1084 — engine-dsp ignored the verified master contract, loaded whole files into memory, and never called Gemini or persisted provenance
  - **Status:** ✅ FIXED (production verified 2026-07-20) — the private worker revalidates the verified generation/hash/owner contract, performs open-source DSP and Vertex Gemini analysis from the canonical `gs://` reference, and writes a server-owned owner-readable receipt. Real FLAC and 24-bit WAV/WAVEX masters completed; duplicate replay returned the existing receipt without a second Gemini run. The deployed read callable was raised to 512 MiB after production memory evidence, and cross-owner receipt reads are denied. Local Firebase/renderer contracts (14), Python worker tests (6), and prior Firestore emulator boundary coverage pass.
...
- [x] ISSUE-1007 — "Cover Art" mode promises distributor compliance but never verifies delivered file
  - **Status:** ✅ FIXED (2026-07-20) — the delivered canonical JPEG/PNG bytes are content-addressed, immutable, measured for RGB/square/3000px/minimum requirements, and re-audited server-side before packaging. A compliant Creative Studio cover carries its recorded Google provider/model evidence from generated history into the selected release cover, canonical reference, durable release record, and immutable audit receipt; no provider version is fabricated when the upstream response provides none. A failed server audit preserves the release draft, blocks delivery, and presents a visible replacement-cover action. Focused selector, repair, canonical-cover, and audit tests pass; renderer typecheck and scoped lint pass.
