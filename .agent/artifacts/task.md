...
- [ ] ISSUE-784 — DDEX compiler emits fake DPID + ERN 4.2 doc app can't actually deliver
  - **Status:** PARTIAL (2026-07-17) — all active Python delivery/package paths now require configured sender/recipient DPIDs and XSD-mode validation before SFTP or `delivery_ready`; the Spotify batch manifest also requires its choreography namespace/XSD. Remaining before closure: consolidate or retire the secondary TypeScript compilers, provision the founder's issued DPID/licensed XSDs, and prove partner acceptance with a real test delivery.
