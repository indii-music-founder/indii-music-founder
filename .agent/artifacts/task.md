...
- [ ] ISSUE-784 — DDEX compiler emits fake DPID + ERN 4.2 doc app can't actually deliver
  - **Status:** PARTIAL (2026-07-20) — all active Python delivery/package paths require configured sender/recipient DPIDs and XSD-mode validation before SFTP or `delivery_ready`; desktop submission stages the upload-once Firebase master, verifies SHA-256 in Electron and Python, derives the DDEX MD5 from the delivered bytes, and sends an XML-plus-resources package directory. The reachable Firebase-only compiler/stager is retired and now fails closed. Remaining before closure: stage/verify canonical cover art, provision the founder's issued DPID/licensed XSDs and recipient profile, and retain partner acceptance from a real test delivery. Founder actions are in `docs/RELEASE_CHECKLIST.md`.
