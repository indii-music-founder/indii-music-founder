...
- The `.agent/test_ledger/OPEN_ISSUES_V2.md` ledger must exist at `.agent/test_ledger/OPEN_ISSUES_V2.md`.
...
4. Log the fix: add a new `ISSUE-NNN` entry to `.agent/test_ledger/OPEN_ISSUES_V2.md` marked `✅ FIXED`.
...
curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
  "https://api.github.com/repos/indii-music-founder/indii-music-founder/pulls?state=all&per_page=10" \
  | python3 -c "
import sys, json
d = json.load(sys.stdin)
for p in d:
...
```bash
curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
  "https://api.github.com/repos/indii-music-founder/indii-music-founder/pulls/<PR_NUMBER>/comments?per_page=100" \
  | python3 -c "
import sys, json
...
```bash
curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
  "https://api.github.com/repos/indii-music-founder/indii-music-founder/pulls/<PR_NUMBER>/reviews" \
  | python3 -c "
import sys, json
...
- For any ❌ FAIL, add a REGRESSION entry to .agent/test_ledger/OPEN_ISSUES_V2.md.
...
## Phase 6: Update .agent/test_ledger/OPEN_ISSUES_V2.md

### Step 16 — Mark newly fixed issues

For each issue fixed in this sweep, update its status line in `.agent/test_ledger/OPEN_ISSUES_V2.md`:
...
4. File any new regressions to `.agent/test_ledger/OPEN_ISSUES_V2.md` as `[REGRESSION]` entries.
...
| `.agent/test_ledger/OPEN_ISSUES_V2.md` updated with new fix statuses | ✅ |
...
│  Phase 6: Update .agent/test_ledger/OPEN_ISSUES_V2.md ledger                                 │
