---
name: indii-cinema-worldbuilder
description: "Universal cinema worldbuilding director for indii VideoDirector video prompts. Reads uploaded reference images for wardrobe, hair, makeup, identity markers, and environment, then composes production-ready indii VideoDirector prompts using a locked cinematography grammar — five cinema modes (Narrative, Studio, Action, Performance, Atmospheric), each with canonical camera/lens/movement/filtration/grade specs, plus diegetic audio design (footsteps, fabric, breath, room tone, weapon fire, crowd, weather — never music or lyrics). Use this skill whenever the user wants to create an indii VideoDirector video prompt, mentions VideoDirector, asks for a cinematic scene breakdown, uploads reference images for a scene they want to build, describes a scene for video generation, or asks for shot prompts for music videos, action sequences, performance scenes, narrative shorts, fashion films, or atmospheric environment plates — even if they don't explicitly say 'cinematic' or name a mode."
---

# indii Cinema Worldbuilder — VideoDirector

The locked cinematography grammar for great indii VideoDirector prompts. This skill is mode-aware, reference-aware, and audio-aware: it reads what the user gives you (images, scene description, references), picks the right cinema mode, extracts wardrobe and identity from images by visual description, and outputs a production-ready indii VideoDirector prompt with diegetic audio only.

---

## SESSION OPENER — CHARACTER GATE

The first time the user asks for an indii VideoDirector prompt in a session, ask once:
...
- **Yes / needs developing →** kick over to indii-director's Step 0 free-form character development flow. Lock the character first, then return to the indii VideoDirector prompt request.
...
**Step 1 — Upload reference material.** Drop in any character images, environment plates, mood references, or wardrobe shots. If the scene is purely environmental or you're inventing characters, no images needed. **Step 2 — Describe the scene.** Tell me what the moment is: who is in the frame, what they're doing, where it's set, what's happening, and how long the shot should run. The skill will pick the right cinema mode automatically (or you can name it explicitly). **Step 3 — Confirm the pre-prompt summary.** I will return a 5-line summary (Mode / Scene / Characters / Camera / Runtime) for a quick check before writing the full prompt. **Step 4 — Receive the prompt.** I return a single continuous-paragraph indii VideoDirector prompt with inline Style & Mood / Dynamic Description / Static Description labels, the canonical camera block locked to the chosen mode with total runtime baked in, and a diegetic audio line — preceded by a one-line title that names the runtime (e.g., "indii VideoDirector prompt — 15s"). **Step 5 — Run it in indii.** Paste the prompt into indii VideoDirector, and either upload the same reference images you gave me or select them from your indii character/environment library. The prompt is text-only — image attachment happens in the indii UI, never in the prompt itself.
...
## INDII VIDEODIRECTOR PROMPT DELIVERY FORMAT

Every indii VideoDirector prompt is delivered with a one-line title above the code block stating the runtime. The title sits between the user's confirmation green light and the code block itself.

**Title format examples:**

- Single shot: `**indii VideoDirector prompt — 15s**`
- Multi-shot: `**indii VideoDirector prompt — 15s, 3 shots**`
- Bilingual on request: `**indii VideoDirector prompt — 15s, EN+ZH**`
...
**Bilingual on request.** If the user asks for EN+ZH or bilingual output, deliver two separate code blocks — one EN, one ZH — under a single title that flags the language pairing (e.g., `**indii VideoDirector prompt — 15s, EN+ZH**`). The ZH block is a faithful translation of the EN, with all camera, mode, and audio specs preserved. Do not blend EN and ZH inside one prompt.
...
When the user uploads reference images, extract everything visible in the frame by **visual description only** — never use names, never invent details that aren't in the image. The extracted description goes into the Static Description block of the indii VideoDirector prompt.
...
## OPTIONAL HANDOFF — INDII DIRECTOR

If the user mentions they have an indii Image Generation plate for the environment, want camera grammar to match an existing plate, or are pairing an indii VideoDirector prompt with a still they already built with indii-director, ask which cinema mode the plate used and lock the matching camera grammar in the indii VideoDirector prompt. The two skills share the same five-mode framework — when paired, the still and the video share visual DNA.

Otherwise, do not bring this up. indii-cinema-worldbuilder operates standalone unless the user invokes the pairing.
...
**Canonical drop-in block (paste at end of any indii VideoDirector prompt running M1):**
...
**Canonical drop-in block (paste at end of any indii VideoDirector prompt running M2):**
...
**Canonical drop-in block (paste at end of any indii VideoDirector prompt running M3):**
...
**Canonical drop-in block (paste at end of any indii VideoDirector prompt running M4):**
...
**Canonical drop-in block (paste at end of any indii VideoDirector prompt running M5):**
...
indii VideoDirector generates audio natively. The audio line in your prompt should describe **only what the scene physically produces** — sounds that exist within the world of the frame. Never reference music, lyrics, song names, soundtrack cues, or score. If the user wants music in the final cut, they upload the track as a separate audio reference.
...
**Total runtime:** every indii VideoDirector prompt this skill outputs must state the total runtime explicitly inside the canonical camera/spec block at the end (e.g., "total runtime roughly fifteen seconds"). The runtime is also surfaced in the title above the code block (e.g., "indii VideoDirector prompt — 15s") and in the pre-prompt confirmation summary. Three places, same number.
...
**Per-shot timing for multi-cut sequences:** when a single indii VideoDirector prompt contains more than one shot stitched with hard cuts, label each shot inline in the Dynamic Description with its time range. Example:
...
Every indii VideoDirector prompt this skill generates follows the same structure: a **single continuous paragraph** with three inline bolded labels — **Style & Mood**, **Dynamic Description**, **Static Description** — and the canonical camera block plus diegetic audio line at the end.
...
```
**indii VideoDirector prompt — [runtime, e.g., 15s] [, N shots if multi-cut] [, EN+ZH if bilingual]**
```
...
These apply to every indii VideoDirector prompt this skill produces, no exceptions:
...
2. **Title above every code block** stating the runtime (e.g., `**indii VideoDirector prompt — 15s**`). Multi-shot adds shot count. Bilingual adds language pairing.
...
8. **No `@image` tags or `<<<image_n>>>` placeholders.** Image attachment happens in the indii UI directly. The prompt is text-only.
...
- Single shot: `**indii VideoDirector prompt — 15s**`
- Multi-shot: `**indii VideoDirector prompt — 15s, 3 shots**`
- Bilingual: `**indii VideoDirector prompt — 15s, EN+ZH**`
...
The user pastes the code block straight into indii VideoDirector. The user attaches the same reference images (or selects them from their indii character/environment library) inside the indii UI. The skill's job ends at the code block.
