...
- **PREVENTION:** Any feature that executes generated documents inside srcdoc/blob iframes must treat the parent page CSP as its own sandbox ceiling. CORRECTED after live verification (2026-09-12): an initial read of this incident claimed Firebase header precedence was first-match and the sidecar URLs were edge-poisoned — BOTH were artifacts of probing the wrong hosting site. Domain map: `indii.music` + `founder.indii.music` → LANDING site (`indii-music-founder`); the STUDIO lives at `app.indii.music` (site `indii-music-studio`, also `indii-music-studio.web.app`). On the app target the `/creative/**` header block applies correctly (COOP `same-origin`, scoped `blob:` live) and the sidecars serve as real JS. Real gotchas worth keeping: (1) custom headers apply by REQUEST URL even when an SPA rewrite answers, so `.js` URLs carry `immutable, max-age=1y` — ship changed assets under a versioned `?v=` key (lockstep-enforced by `sidecarServing.test.ts`); (2) ALWAYS confirm the domain→site mapping (`.firebaserc` targets) before diagnosing production headers — landing HTML on a studio route looks exactly like a CSP misconfiguration. Before claiming preview works, verify under a meta-CSP replica of the production header (see CSP-replica harness pattern in ISSUE-1433 ledger note), not just on the CSP-less dev server. Related hardening: compiler script-context JSON must use `jsonScriptSafe` (escapes `<`, U+2028, U+2029) — plain `JSON.stringify` lets `</script>` in user strings break out of inline scripts.
...
- SECOND follow-up (same day): clicking a VIDEO in Project Assets opened the IMAGE 'magic edit' canvas (wrong editor) — confusing + blank stage. FIX: `AssetsPanel.handleAssetClick` now routes video -> `setGenerationMode('video') + setViewMode('video_production')` (VideoWorkflow stage plays the clip via VideoJsPlayer); images still open the editor. Also `VideoJsPlayer` now falls back to a native `<video>` when video.js errors, so the stage plays rather than a dead red box.
...
## 2026-08-21 Mobile Remote: receipt-stamped freshness let dead Studios look alive (cache-hit forgery)

- SEVERITY: High (the Controller's recurring "pairs, shows Active for ~2 minutes, drops to Standby" flapping; every reconnect attempt reset the lie)
- FILES: `packages/renderer/src/services/agent/RemoteRelayService.ts`, `packages/renderer/src/hooks/useRemoteCommandListener.ts`, `packages/renderer/src/modules/mobile-remote/MobileRemote.tsx`
- ERROR: `onDesktopState` stamped every snapshot with `_localReceivedAtMs = Date.now()` and the freshness predicates trusted that stamp OVER the doc's server heartbeat timestamp. Firestore re-serves cached doc content as the first snapshot of EVERY subscription (initial mount, manual retry, each auto-reconnect resubscribe), so an `online:true` doc whose last real heartbeat was hours old was certified "connected" for a fresh 120s window on each resubscribe — then dropped, then "recovered" by the next attempt.
- FIX: Replace the unconditional receipt stamp with heartbeat-ADVANCE tracking (`_heartbeatAdvancedAtMs`): only a snapshot whose server timestamp VALUE changed proves a live heartbeat, stamped on the local monotonic clock. No advance witnessed → freshness must hold on the doc's own timestamp age (+30s skew tolerance) alone. `studioStateFreshnessRemainingMs` mirrors the same boundary so scheduled stale timers cannot disagree with the predicate.
- PREVENTION: Local receive time of a document is evidence about the LISTENER, never about the WRITER. When a cache can replay old content, freshness must derive from monotonic progress of the payload (timestamp value changes), not from wall-clock recency of delivery.

## 2026-08-21 Mobile Remote: silent queues answered remote commands with false completions

- SEVERITY: High (phone showed "Done." for work that had not run; queued messages were silently overwritten; capture receipts cleared the phone's only copy)
- FILES: `packages/renderer/src/services/agent/AgentService.ts`, `packages/renderer/src/hooks/useRemoteCommandListener.ts`
- ERROR: Three stacked defects around desktop busyness: (1) `sendMessage()` queues-and-returns when busy, and the relay chat route translated that into a literal final response of "Done." via its find-newest-response fallback; (2) `pendingSend` was a single slot, so a second queued message discarded the first entirely; (3) `processSingleCommand` set its `isProcessing` lock only AFTER awaiting the async cloud-function claim, letting two near-simultaneous commands pass the guard and run concurrently. Dispatch tasks marked 'completed' (which ISSUE-983 callers treat as "note exists") while merely queued.
- FIX: Chat route reports an explicit QUEUED response when no new agent message exists and the agent is still busy (`shouldReportQueuedChatToRemote`). Queue is a bounded FIFO (25) drained in order; overflow throws instead of silently discarding. Lock is taken synchronously before the first await with a single release point in one finally. Dispatch paths assert the desktop actually ran (`assertDesktopWasFreeToRun`) and otherwise fail loudly so captures are kept.
- PREVENTION: A function that defers work instead of executing it is a DIFFERENT outcome and needs a different response contract. Busy-queue detection belongs in the layer that reports completion, and locks must be acquired before any await that can interleave competing events.

## 2026-08-22 Mobile Remote: the desktop-resident bottom line was only partially reachable from the phone

- SEVERITY: High (founder bottom line: phone must reach files, notes, boardroom through Studio execution; two of three were silently degraded)
- FILES: `packages/renderer/src/services/agent/AgentService.ts`, `packages/renderer/src/hooks/useRemoteCommandListener.ts`, `packages/renderer/src/modules/mobile-remote/components/AgentChat.tsx`, `packages/renderer/src/services/agent/tools/NotesTools.ts`, `SuperpowerTools.ts`, `BaseAgent.ts`, `ToolRiskRegistry.ts`, `packages/firebase/firestore.rules`
...
## 2026-08-25 Talk-over: Web Speech stop() resolves async — a rebind during the stop-in-flight window routes the OLD session's onend into the NEW session (dead mic on release→re-talk)

- SEVERITY: High (the founder-escalated "persistent talk-over system failures"; reproduced by the single most common gesture: release the TalkButton, then click Talk again)
- FILES: `packages/renderer/src/services/intelligence/VoiceService.ts`, `packages/renderer/src/core/components/command-bar/TalkButton.tsx`
- ERROR: `stopDictation()` set `stopRequested` and called `recognition.stop()`, but Chrome resolves the stop asynchronously via `onend`; `isDictating` stayed true until then. A `startDictation()` arriving in that window took the live-rebind branch (guard `if (!this.isDictating)` skipped `recognition.start()`) and OVERWROTE `recognition.onend` with the new session's closure. The old stop's `onend` then fired into the NEW session: natural-end for a session that never started, engine dead, button flickering listening→idle with an empty take.
- FIX: Explicit engine state machine. `engineRunning` tracked via `onstart`/`onend`; `stopRequested` marks the stop-in-flight window; a session arriving mid-stop is QUEUED (`pendingHandlers`) and `beginSession()` starts it from `onend` the moment the engine frees (no `start()` on a stopping engine → no InvalidStateError; no rebind → the old session's end can't leak into the new one). `stopDictation()` discards any queued session.
- SIBLING DEFECTS (same class, same commit): (1) final transcripts concatenated with `+=` fused words when Chrome omitted trailing spaces — now `joinTranscripts` with whitespace-aware separator; (2) TalkButton unmount called `stopDictation()` unconditionally, so an IDLE surface unmounting killed another surface's live session — now `stopDictationIfOwner(handlers)` identity-checked; (3) legacy `startListening` (onboarding) could clobber a live dictation session and left its UI stuck "listening" — it now supersedes the owner and resets `continuous/interimResults` to single-shot config; (4) agent going busy mid-listen left a hot mic with no visible owner — busy stand-down stops owner-scoped and the onEnd path always resets UI state.
- PREVENTION: A browser API whose stop is asynchronous is a state machine with at least three states (running, stopping, idle) — a boolean "listening" flag is a lie. Never rebind handlers across a stop boundary; queue the successor and let `onend` promote it. Component cleanup that touches a shared singleton must be ownership-checked, never unconditional.


## 2026-08-25 Founder: "I'm the founder. I should never see this" — cost reservation denial on a stitch job

- SEVERITY: High (founder-facing hard block on the flagship video path)
- FILES: `packages/firebase/src/functions/billing/enforceOperationCost.ts`, `packages/firebase/src/functions/auth/entitlements.ts`, `packages/renderer/src/services/video/VideoGenerationService.ts`, `packages/renderer/src/services/agent/specialists/GeneralistAgent.ts`, `agents/conductor/skills/video_producer/SKILL.md`
- ERROR: The founder asked the Conductor to stitch clips into a longer video and got a "cost reservation error". Two stacked causes: (1) even a correctly-entitled FOUNDER was NOT exempt from the flat $20 USER_CONFIRMATION_THRESHOLD (a movie stitch estimates over $20 → `requiresConfirmation` → the video client hard-blocks with no prompt UI), and the flat $500 RUNAWAY_LIMIT fired BELOW the founder tier's own $10k/month ceiling — so founder-tier spend past $500/month was killed by the kill-switch itself; (2) entitlement provisioning read ONLY `founders/{uid}` — any paying account whose founders/subscriptions record lagged (e.g. mid-migration) resolved FREE and was budgeted against free limits ($5/day, $50/month, 5 ops/hour). Free-tier reasons then surfaced through the Conductor as an opaque "cost reservation" failure, which `isHardStopError` did not classify, so the model kept paraphrasing/retrying.
- FIX: (1) founder/enterprise tiers bypass the per-op confirmation threshold (their own budget ceilings ARE the gate); the runaway kill-switch now caps at `max($500, tier monthly ceiling)` so it can never fire below a tier's own budget; (2) provisioning now resolves the proven tier from `founders/{uid}` AND a non-canceled paid `subscriptions/{uid}` (`subscription_migration` grant source) and upgrades a stale FREE/recorded entitlement in place when server evidence proves a higher tier (tierRank guard, one-directional); (3) video confirmation blocks get a clear actionable message; `isHardStopError` now stops on 'cost reservation'/'budget'/'confirmation threshold'; (4) the conductor's video_producer skill now hard-routes stitch/join/sequence jobs to the LOCAL pipeline (`inspect_video_project` → `add_video_clip` → `queue_video_render`), reserving Veo for generating NEW footage only — a stitch job must never touch cost reservation when the clips already exist.
- PREVENTION: A tier's own budget limits and every global guard must be ordered consistently — no global cap may fire below any tier's advertised ceiling. Entitlement tier must derive from server-owned registries only, and a recorded entitlement is a cache, not a verdict: re-provision against live evidence, upgrade-only. Agent skill docs need an explicit job→tool routing table so edit jobs never fall through to generation tools.

## 2026-08-25 In-app cinematic treatments (MIG-010) — schema + compiler + conductor tool + editor picker

- SEVERITY: Feature (founder: "I need my app to make that same video — user instruction, not manual")
- FILES: `packages/shared/src/types/videoProject.ts`, `packages/main/src/services/video/hyperframes/compiler.ts`, `packages/renderer/src/services/video/treatmentPresets.ts`, `packages/renderer/src/services/agent/tools/VideoProjectTools.ts`, `packages/renderer/src/modules/creative/video/editor/components/TreatmentPicker.tsx`, `agents/conductor/skills/video_producer/SKILL.md`
- GAP: A user could stitch clips + text on a plain black canvas, but could not say "make it feel like an amber night" — the IndiiVideoProject model had no background, seam, kinetic-entrance, counter, or audio-fade vocabulary, and the conductor had nothing to translate a direction into.
- FIX: (1) shared schema gains `IndiiBackground` (solid/radial-glow/grid/ghost-text), `IndiiSeam` (cut-the-curve + direction), `IndiiEntrance` (waterfall/inverse-zoom), `IndiiCountUp`, `IndiiAudioFade` on clips + `background`/`seam` on projects; (2) the HyperFrames compiler emits background layers with finite ambient tweens, velocity-matched seam tweens at adjacent clip boundaries (power3.in exit / power4.out entry, 12%/10% travel), word-span waterfall arrivals, seek-safe GSAP counters, inverse-zoom arrivals, and absolute-gain volume fade tweens (data-volume=1 when fades present); (3) named presets (`amber-night-cinematic`, `clean-grid`, `bold-arrival`) with a direction-word resolver; (4) conductor tool `apply_video_treatment` + skill routing table mapping mood words → presets; (5) editor toolbar TreatmentPicker using the same resolver as the tool.
- PREVENTION: Direction/intent vocabulary must live in the canonical project model (framework-neutral), with engines adapting TO it. UI pickers and agent tools must share one resolver so a pick and a chat instruction produce identical projects. Compiler-emitted selectors need their `#` prefix verified by a test (this run shipped `"el-c1-box"` → matched nothing until the test caught it).

## 2026-08-26 Cloud render for every user (MIG-010 cloud) — compiler extraction, durable queue, worker, dispatcher, runbook

- SEVERITY: Feature (founder: "every user including the cloud users need to be able to use this hyperframes feature… build everything… write down the specifics about what needs to happen with Google cloud")
- FILES: `packages/video-compiler/*` (new pure package), `packages/render-worker/*` (new), `packages/firebase/src/functions/video/{queueCloudVideoRender,dispatchCloudVideoRender}.ts`, `packages/firebase/src/config/cloudRender.ts`, `packages/firebase/firestore.rules`, `packages/renderer/src/services/video/{CloudVideoRenderService.ts,LocalVideoProjectRenderer.ts}`, `docs/CLOUD_HYPERFRAMES_RUNBOOK.md`
- ARCHITECTURE: (1) the compiler extracted to @indii/video-compiler so electron main, web renderer, and the server worker share ONE translation layer without pulling the Electron tree into containers; (2) durable job protocol `users/{uid}/videoRenderJobs/{jobId}` — callable-created, owner-read-only, one-hop transitions queued→running→completed|failed, advanced only by trusted executors; (3) cloud render worker (Node + chromium + ffmpeg, Cloud Run-shaped) stages media BESIDE the composition, compiles, runs the vendored CLI, uploads to Storage, completes the job; (4) onDocumentCreated dispatcher bridges queue→worker with a shared Bearer secret, degrading to a visible queued state when the URL is unprovisioned; (5) renderer web path queues+polls the protocol when the desktop API is absent, same completed receipt contract.
- ERROR CAUGHT IN TESTING (fail-closed design working as intended): staging media to a DIFFERENT directory than index.html produced 0% video-frame coverage — the CLI's own coverage gate aborted the render rather than shipping a blank MP4; fixed by one shared composition directory, and the worker's artifact-exists check covers the remainder. Also the counter identifier bug class (hyphens in JS identifiers) was again caught by real-CLI lint before ship.
- PREVENTION: (1) every executor shares the compiler via a dependency-free package — never import Electron-main code from a server container; (2) job statuses are one-hop and server-owned — clients read, callables/executors write; (3) a dispatch layer must degrade to queued+logged when its downstream is unprovisioned, never invent a terminal state; (4) GCP activation lives in a copy-pasteable runbook with explicit cost rails (max-instances, concurrency 1, timeout) and rollback.

## 2026-08-26 GCP activation blocked: Google revoked the machine's refresh tokens (invalid_rapt)

- SEVERITY: Blocker (not a code defect — an external auth revocation)
- FILES: none (docs/CLOUD_HYPERFRAMES_RUNBOOK.md updated with Step 0 reauth)
- ERROR: Executing the authorized GCP runbook failed at the credential wall: gcloud user tokens expired AND Google rejected the stored firebase-CLI refresh token with `invalid_grant / invalid_rapt` — reauthentication required. No offline path can mint a token Google revoked.
- FIX: Interactive reauth with the founder present: `gcloud auth login`, `gcloud auth application-default login`, `firebase login:reauth`. Then resume `docs/CLOUD_HYPERFRAMES_RUNBOOK.md` at Step 3.
- PREVENTION: Long-lived automation on GCP needs a dedicated service-account key (stored in the repo's secret manager / CI secrets), not user refresh tokens — user OAuth revokes under Google's security policy (rapt) and takes every local workflow down with it. After activation, mint a scoped SA key for the runbook's own future automation.

## 2026-08-26 Professional editor round (MIG-010b) — text styling, speed, edit ops, undo, snapping, presets

- SEVERITY: Feature (founder: "keep building presets and things for the video editing portion just needs to be a professional video editor")
- FILES: `packages/shared/src/types/videoProject.ts`, `packages/video-compiler/src/compiler.ts`, `packages/renderer/src/modules/creative/video/store/videoEditorStore.ts`, `editor/{VideoEditor,VideoPropertiesPanel,VideoPropertySections,TimelineClip,TimelineTrack,TrackList,VideoTimeline}.tsx`, `editor/hooks/{useTimelineDrag,useVideoEditor.undoRedo,useVideoEditor.splitDuplicate}.ts`, `services/video/treatmentPresets.ts`, conductor skill
- DELIVERED: (1) professional text: embedded-font allowlist (fail-closed), letter spacing, case, caption panels, shadows; (2) constant playbackRate 0.25–4× (clamped, data-playback-rate); (3) razor split (source-µs-aware), duplicate, delete; (4) undo/redo (capped 50, recorded through the store's single set wrapper, ⌘Z/⌘⇧Z); (5) timeline snapping to playhead/neighbors/boundaries + source-aware left/right trim handles; (6) aspect-ratio presets (16:9/9:16/1:1/4:5); (7) nine treatment presets total.
- ERROR CAUGHT IN TESTING: the undo history added via the store wrapper broke VideoEditor interaction tests (stub lacked past/future) — fixed by extending the stub; snap float equality in µs math → toBeCloseTo. No production defects shipped.
- PREVENTION: history recording must live in ONE store write path (the wrapper) so every future action is covered automatically; timeline geometry math must stay in pure exported functions so snapping/trimming stay unit-testable without a browser.

## 2026-08-27 Professional editor round 2 — ripple delete, zoom, loop region, desktop relay

- SEVERITY: Feature (founder: "proceed reel is done" → finish the professional editor + close every named gap)
- FILES: `videoEditorStore.ts` (rippleDeleteClip, loopRegion + setCurrentTime clamp), `VideoEditor.tsx` (toolbar: zoom, loop, ripple), `TimeRuler.tsx`/`TimelineClip.tsx`/`TimelineTrack.tsx`/`VideoTimeline.tsx` (zoom scaling), `useTimelineDrag.ts` (zoom-aware deltas + snapping), `packages/firebase/src/functions/video/relayVideoRenderJob.ts` (claim/complete callables), `packages/main/src/handlers/video.ts` (video:read-artifact IPC, scope-checked), `DesktopRenderRelayService.ts` (desktop executor of queued jobs)
- KEY DECISIONS: loop enforcement lives in setCurrentTime (snaps while playing, unrestricted while paused) — one clamp point, no player-element coupling; ripple delete shifts only same-track clips at/after the gap end; zoom is a single store multiplier consumed at every pixel math site (clip widths, ruler, click-seek, drop position, keyframe placement, drag deltas); the relay's artifact read is an IPC whose scope/extension/size checks mirror video:render's security block.
- PREVENTION: pixel↔frame conversions must all derive from one zoom-aware pxPerFrame, never the raw constant; queue executors (cloud worker AND desktop relay) must both enforce the one-hop status protocol server-side, so a second executor cannot resurrect or double-run a job.

## 2026-08-27 Agent chat 413 payload guard — raw base64 attachments blew the server cap; persona layer laundered the failure into a bogus "verdict"

- SEVERITY: P1 (creative workflow with large images unusable; agent reported a misleading authoritative verdict instead of the real error)
- FILES: `packages/firebase/src/index.ts` (200K-char contents guard, pre-existing), `packages/renderer/src/services/agent/AgentService.ts`, `packages/renderer/src/services/agent/BaseAgent.ts`, `packages/renderer/src/services/intelligence/FirebaseIntelligenceService.ts`, `packages/renderer/src/services/intelligence/StreamPayloadGuard.ts` (new), `packages/renderer/src/shared/types/errors.ts` (PAYLOAD_TOO_LARGE), `packages/renderer/src/services/persona/PersonaAgentResponseService.ts`
- ERROR: Creative-agent consultation failed with HTTP 413 "Content payload is too large." from generateContentStream's server guard (`JSON.stringify(contents).length > 200_000`). The client mapped the 413 to a generic INTERNAL_ERROR, `consult_specialist` fed the raw text back into the conductor, and the Evolas persona finalizer (verdict/riskLevel/caveats/escalate schema) rendered the failure as an authoritative "Operational Verdict Report" complete with improvised trademark/IP caveats.
- CAUSE: Three stacked gaps. (1) Chat attachments crossed as RAW base64 inlineData — no compression anywhere in the agent path (`AgentService.handleDirectChatFlow`, `BaseAgent` requestContents). (2) `BaseAgent` auto-injects the latest generated artifact as a full data-URL for creative/brand agents — one generated PNG (0.5–3MB → 0.7–4M base64 chars) alone exceeds the 200K-char guard 3–20×. (3) Client-side guards measured images as FLAT TOKENS (`TokenEstimator` 258/image, `ContextManager` 500/image) while the server limit is CHARACTERS — unit mismatch, so every pre-flight passed.
- FIX: New `StreamPayloadGuard` mirrors the server measurement exactly; `compressStreamImageAttachments` walks a 1024→768→512 JPEG ladder via the existing certified `CloudStorageService.compressImage`, fail-open to the original on compression error; `assertContentsWithinStreamBudget` throws the new `PAYLOAD_TOO_LARGE` locally BEFORE fetch (BaseAgent converts it to a controlled honest halt, not an opaque mid-consult throw); FirebaseIntelligenceService maps HTTP 413 to the same code; `finalizePersonaAgentResponse` now passes failed-execution responses (`response.error` set) through byte-identical, exactly like tool-bearing ones — failure text can never become a persona verdict again.
- EVIDENCE: 1778 passed / 0 failed across services/{agent,intelligence,persona}; monorepo `npm run typecheck` clean; eslint 0 errors on all touched files. Local structural evidence only — no live-user run.
- PREVENTION: A client mirror of a server guard must use the SAME UNIT as the guard — flat per-image token approximations cannot defend a byte budget. Persona/advisory layers must only style COMPLETED specialist analyses; executor halts must carry a distinct machine-readable error field that the presentation layer passes through untouched. If a guard fires in production, the surfaced message must name the real mechanism, or the model will improvise one.
- FOLLOW-UP RESOLUTION (2026-08-29, `932433c3c`): TokenEstimator inlineData parts no longer count flat 258 tokens — they scale with serialized size, floored at 258 (6 unit tests). Stream-caller audit closed: all generateContentStream callers funnel through FirebaseIntelligenceService.rawGenerateContentStream -> callBackendGenerateContentStream, which asserts the 200K-char budget; no bypass path. Canvas image-editor + memory-ingestion size limits remain OPEN. 2026-08-29 'get it all done' delivery: plan workstreams B1/B2/C1-core/C2/I1/H2 + A1 partial shipped to origin/main (each green at its SHA). A1.1/A1.5 calibration, C2.3 @imgly license, and all real-smokes remain founder-blocked (recorded in HANDOFF). — no server contract to mirror, no product limit specified; founder input required.
- FOLLOW-UP (same day, founder challenge "the too-large images are also images the app made"): a SECOND live path existed OUTSIDE inlineData — the tool loop embeds each tool result into the next iteration's prompt (`fullPrompt += [Tool Call: name(args)] Result: ${outputText}`), and `outputText` for a tool result without a short message is `JSON.stringify(result.data)` — for `generate_image` that is the app-generated image as a full data-URL, re-entering as prompt TEXT megabytes at a time, unreachable by inlineData compression. Tool ARGS can also carry base64 (annotation edit tools). FIX: `elideBase64Payloads` sanitizer (mime-preserving, size-reporting, ≥1KB threshold so test fixtures pass) applied at every `fullPrompt +=` site in BaseAgent (args JSON + outputText). Evidence: 1782/0 across services/{agent,intelligence,persona}; typecheck clean; lint 0 errors. LESSON: a guard on the SERIALIZED request is a backstop, not a design — audit every path that builds the request TEXT, not just the attachment parts.

## 2026-08-28 Studio Video Editor black preview — DOM-ownership wrapper lost the width contract

- SEVERITY: P1 (core video editor preview displayed as a black/empty box)
- FILES: `packages/renderer/src/modules/creative/video/components/VideoJsPlayer.tsx`, `VideoJsPlayer.test.tsx`, `e2e/video-preview-display.spec.ts` (new), `packages/renderer/public/e2e/sample-clip.mp4` (fixture)
- ERROR: after `37628bddc` ("isolate video player DOM ownership"), the Video Editor preview rendered as a black/empty box. Browser probe proved the video itself was HEALTHY (`videoWidth:640, readyState:4, error:null`) while the player bounding box was **2x3px** — the 2px being the component's own border.
- CAUSE: the refactor replaced the React-owned `<video class="video-js">` root with an intermediate container `<div className={className}>`. Callers size the player through `max-h-full max-w-full` — CONSTRAINTS, not a width source. In the stage's centered flex context the div shrink-wraps; video.js fluid sizing (`width:100%` + aspect padding) then resolves against the degenerate box. The old root had been the `.video-js` element itself, taking width directly from the parent.
- FIX: container reproduces the old root's layout contract — `className={`block w-full ${className}`}` — plus unit assertion on the class contract and a structural e2e regression spec (`video-preview-display.spec.ts`) that injects a real mp4, asserts bounding box >200px wide and decoded frames.
- EVIDENCE: Playwright before/after — before: rect 2x3.125px, video.js width 0px/padding-top 0px with a fully decoded stream; after: full-size player (screenshot shows test pattern rendering in the stage), e2e spec green, 236/0 video unit tests, typecheck clean, lint 0 errors. Structural evidence (mocked auth + injected store) — real-user confirmation pending founder.
- PREVENTION: when a refactor swaps a component's root element, the NEW root must reproduce the OLD root's LAYOUT contract, not just its behavior contract — sizing (display/width/height) is part of the public contract of a visual component. `max-w-*`/`max-h-*` are constraints and can never substitute for a width source when children size in percentages.

## 2026-09-05 COEP ERR_BLOCKED_BY_RESPONSE for firebaselogging-pa (NotSameOriginAfterDefaultedToSameOriginByCoep)

- SEVERITY: High (104+ red console errors per session; telemetry retry loops caused network noise and console spam)
- FILES: `firebase.json` (lines 180 & 225), `packages/renderer/public/bootstrap.js`, `packages/renderer/src/bootstrap.test.ts`
- ERROR: `POST https://firebaselogging-pa.googleapis.com/v1/firelog/legacy/log net::ERR_BLOCKED_BY_RESPONSE.NotSameOriginAfterDefaultedToSameOriginByCoep 200 (OK)` (initiator `@firebase/app-check` / Firebase Performance). 
- CAUSE: `/creative` and `/creative/**` headers in `firebase.json` configured `Cross-Origin-Embedder-Policy: require-corp` to enable `SharedArrayBuffer` for media processing workers. Google's firelog endpoint responds with HTTP 200 without a `Cross-Origin-Resource-Policy: cross-origin` header. Under `require-corp`, Chrome blocks the response, causing Firebase Performance / App Check to treat the telemetry dispatch as failed, re-queue the payload, and continuously retry every 10 seconds.
- FIX:
  1. Updated `firebase.json` for `/creative` and `/creative/**` from `Cross-Origin-Embedder-Policy: require-corp` to `Cross-Origin-Embedder-Policy: credentialless`. Modern standard `credentialless` maintains `crossOriginIsolated` and `SharedArrayBuffer` support while permitting cross-origin subresources without credentials to load without CORP headers.
...
## 2026-09-26 Asset Version Graph Had No Firestore Rule — Every Artifact Finalization permission-denied (ASSET_VERSIONS_MISSING_FIRESTORE_RULE)

- **SEVERITY:** Critical (entire Artifacts feature dead in production: `record_asset_version`, `promote_asset_version`, and every tool-side `AssetVersionService.recordVersion` call (canvas_export, mockups, distribution bundles, video/image exports) failed; agents then confabulated "DevOps escalations" to cover the failure)
- **FILES:** `packages/firebase/firestore.rules`, `packages/firebase/src/test/security/firestore.rules.test.ts`, `packages/renderer/src/services/agent/builders/AgentPromptBuilder.ts`, `packages/firebase/src/relay/agentPrompts.ts` (write path `packages/renderer/src/services/assets/AssetVersionService.ts` unchanged)
- **ERROR:** Creative Director's `record_asset_version` failed with a permissions error on the asset registry — Firestore `permission-denied` on `setDoc`. Artifacts UI stayed empty; `promote_asset_version` died too (its `getVersionTree` READ hits the same deny before it can find any version).
- **CAUSE:** `AssetVersionService` persists to `users/{uid}/assetVersions/{assetId}/versions/{versionId}`, but `firestore.rules` had NO match for `assetVersions` — the write fell through to the final catch-all `match /{document=**} { allow read, write: if false; }`. The plan doc (`docs/CREATIVE_FINALIZATION_TOOLS_PLAN.md` §613) said to "mirror LikenessService exactly"; Likeness got its rule (line ~991), the mirror never did. This is the SAME failure class as ISSUE-1359 (canvases/storyboards/concept_art) and the 2026-07-12 unruled-collections sweep (ISSUE-842/944/998/1000/1001) — `assetVersions` was simply missed by both sweeps.
- **SECONDARY (agent honesty):** With the tool failing, indii Conductor invented a "DevOps escalation via the direct-to-dev pipeline". No such pipeline exists anywhere in the codebase (no tool, queue, function, or Firestore record) — pure confabulation. Contributing factor: the capability-truth prompt asserts all departments are "fully implemented and operational", which nudges agents toward narrating plausible internal processes instead of reporting raw failure.
- **FIX:**
  1. Added `match /users/{userId}/assetVersions/{assetId}/versions/{versionId} { allow read, write: if isOwner(userId); }` to `firestore.rules` (next to the sibling owner-scoped subcollection rules).
  2. Added 5 emulator rules tests (owner record/read allowed; cross-user write/read denied; unauthenticated write denied) in `firestore.rules.test.ts`.
  3. Hardened BOTH execution-contract copies (renderer `AgentPromptBuilder.buildExecutionContract` + relay `agentPrompts.ts` EXECUTION_CONTRACT, per its SYNC NOTE): TOOL-FAILURE HONESTY (quote raw error, never reframe failure as success) and NO PHANTOM ESCALATIONS (only claim escalation/handoff if a real tool call performed and evidenced it).
- **EVIDENCE:** Emulator rules suite 257 passed (2 files); targeted `assetVersions` block 5 passed / 249 skipped — owner create allowed, BOB/unauthenticated denied at the new rule line (L1808) plus catch-all. Firebase package lint 0 errors, `tsc` build clean.
- **PREVENTION:** Any service that introduces a new Firestore path MUST ship its rules match AND emulator rules test in the same change — grep `firestore.rules` for the collection name before merging anything that calls `setDoc`/`getDocs`. Deploy with `firebase deploy --only firestore:rules` (owner-triggered). Treat fabricated ops-narratives ("I escalated to X") from agents as a review-blocking anti-pattern in the same class as fabricated roadmaps: an agent may only claim a process happened if a tool call evidences it.

## 2026-09-26 Raw Tool Errors Leaked to Subscribers; No Error-Report Channel Existed (AUDIENCE_UNAWARE_TOOL_ERROR_RELAY)

- **SEVERITY:** Medium (privacy/trust: internal infrastructure detail was instructed to be quoted verbatim into PAID subscriber chats; subscribers had zero way to report a failure)
- **FILES:** `packages/renderer/src/services/agent/builders/AgentPromptBuilder.ts`, `packages/renderer/src/services/agent/definitions/SuperpowerTools.ts`, `packages/renderer/src/services/agent/tools/ErrorReportTools.ts` (new), `packages/renderer/src/services/agent/BaseAgent.ts`, `packages/renderer/src/services/agent/ToolRiskRegistry.ts`, `packages/firebase/firestore.rules`, `packages/firebase/src/test/security/firestore.rules.test.ts`, `packages/firebase/src/relay/agentPrompts.ts`
- **ERROR:** Founder directive: raw technical error feedback must be founder-only; paid subscribers must get a clean failure explanation AND a working way to report the error.
- **CAUSE:** The ISSUE-1444 honesty hardening added a universal "quote the raw error message verbatim" clause to the shared EXECUTION CONTRACT with no audience distinction, and the platform had no error-report intake (no tool, no collection, no rules).
- **FIX:** Audience-aware contract (`founderDiagnostics` from `isFounderUserProfile`: flag or 'founder' tier) — founder keeps verbatim quoting; subscribers get plain-language failures, a hard ban on relaying raw text/codes/paths/service names, and the new `report_error` superpower tool that files an immutable owner-scoped `errorReports/{reportId}` doc (rule + 7 emulator tests shipped together per ISSUE-1444 prevention; admin read for triage; update/delete false). Relay contract copy made subscriber-safe by default. TS gotcha worth remembering: `wrapTool<TArgs extends ToolFunctionArgs>` rejects `interface` arg types (no implicit index signature) — use a `type` alias.
- **EVIDENCE:** Emulator rules 264/264; scoped renderer suites green; root typecheck 0 errors.
- **PREVENTION:** Any prompt clause that governs what agents RELAY TO USERS must be audience-aware or written at the strictest-audience level. Never add a universal "expose internals" instruction to shared contracts. Any subscriber-facing failure path needs an intake channel with its Firestore rule + emulator test in the same change.

## 2026-09-26 reportBugFn Deployed Without GITHUB_REPO Env or Secret Binding — Every Bug Report Silently Stranded in Firestore

**SEVERITY:** High (the app's bug→GitHub pipeline delivered zero issues for months; invisible to CI)

**MISTAKE:**
- FILES: `packages/firebase/src/functions/agent/reportBugFn.ts` (source was fine), live function config (Cloud Run service `reportbugfn`), GCP Secret Manager `GITHUB_TOKEN`
- ERROR: The deployed `reportBugFn` revision had `GITHUB_REPO` unset and NO secrets mounted (`secretEnvironmentVersions: null`), and the stored `GITHUB_TOKEN` was revoked (GitHub API: 401 Bad credentials). Net effect: every in-app `report_bug` saved to Firestore `bug_reports` then the function threw `failed-precondition` — GitHub never received a single issue. 7 reports stranded (2026-09-24), including two explicit pipeline-verification tests that FAILED SILENTLY. The client logged "GitHub sync failed" but nothing surfaced it. Truthfulness critiques (issue #317) ended up hand-filed by side-agent accounts instead of coming from the app.
- CAUSE: A non-firebase-tools deploy path (raw gcloud/automation) created a revision from source that declares `{ secrets: [githubToken] }` but never bound it, and nothing verified the composite path end-to-end. Each layer looked healthy in isolation: function ACTIVE, secret EXISTING in Secret Manager, code typechecked, CI green. The failure lived only in the runtime binding — the one layer no test or pipeline inspects.
- FIX: (1) Stored a fresh `wiil-tech` PAT as `GITHUB_TOKEN` v2, disabled dead v1. (2) Bound the secret + `GITHUB_REPO` to the live revision via the Cloud Run v2 API (gcloud `--set-secrets` crashed with a client bug: "Invalid secret path ... in annotation" — patch `template` over REST instead). (3) Source fix `ebd06ad8e`'s parent (`5504361c9`): `GITHUB_REPO` falls back to the canonical repo slug so env-less deploys can't break delivery. (4) Follow-up `ebd06ad8e`: sync failures now toast visibly; overclaim auto-reports ride the repaired pipeline.
- EVIDENCE: Token validated via `/user` + repo issues API (200, login `wiil-tech`); live revision Ready with `secretKeyRef` mount; drained report filed as issue #319 authored by `wiil-tech` in app format.
- PREVENTION: Config-level green is NOT delivery proof for external integrations — verify the composite path (real token → real API → visible artifact) after ANY function deploy touching secrets/env. When the firebase CLI deploy path is bypassed, declared secrets silently don't bind. A client log line is not observability: pipeline failures must reach a user-visible surface. gcloud `functions deploy --set-secrets` has a client-side validation bug for same-name bindings — use the Run v2 REST PATCH on `template` with `secretKeyRef`.

**ADDENDUM 2026-09-26 (same regression family, deeper root):** `BugReportTools.report_bug`'s client-side Firestore write to `bug_reports` was DENIED in production the entire time — that collection has no rule and falls into the deny-all catch-all (`git log -S bug_reports -- firestore.rules` shows a rule never existed). The tool's catch block said "Non-blocking — still return success to the agent," so agents told users "documented and prepared for triage" while NOTHING was persisted (verified live: zero `bug_reports` docs, zero callable invocations, zero GitHub issues after an in-app report request). The only durable path was ever the `reportBugFn` Admin-SDK write (how the 2026-09-24 docs landed). Fix: client write removed (callable = single write path), tool now returns `toolError('...NOT filed...')` and fires an error toast unless the callable confirms `firestore: 'ok'` — an agent must never claim a filing that did not happen. RULE: a tool that swallows a persistence failure and returns success converts every downstream "done" claim into a lie; persistence tools must gate success on durable-write confirmation. NOTE: `request_feature` still has the same dead client-side write pattern (no `feature_requests` rule) — known open follow-up, not yet fixed.

## 2026-09-26 Road Agent Maps Tools Left Fail-Closed Stubs After the Secured Backend Shipped; Tour Parameters Waypoints Desynced From the Map

- **SEVERITY:** High (the Road Director — the flagship touring agent — could never search places or compute distances even though the secured backend and provisioned keys existed; the user-visible Planning map silently ignored new waypoints)
- **FILES:** `packages/renderer/src/services/agent/tools/MapsTools.ts`, `packages/firebase/src/lib/touring.ts`, `packages/firebase/src/index.ts`, `packages/renderer/src/services/agent/definitions/{RoadAgent,HospitalityAgent,EventPlannerAgent}.ts`, `packages/renderer/src/modules/touring/RoadManager.tsx`, `packages/renderer/src/modules/touring/components/PlanningTab.tsx`, `packages/renderer/src/modules/touring/hooks/useTouring.ts`, `packages/renderer/src/services/touring/TouringService.ts`, `agents/road/prompt.md`
- **ERROR:** Two failures, one chat. (1) `MapsTools` returned `MAPS_BROWSER_DISABLED` unconditionally for `search_places`/`get_place_details`/`get_distance_matrix` — a deliberate fail-closed from the 2026-08-09 "declarations are contracts" hardening that was never revisited after `findPlaces` (auth + App Check + `GOOGLE_MAPS_API_KEY` secret) shipped and the 2026-08-18 audit proved Geocoding/Places live with CI-injected keys. The Road Director therefore honestly-but-uselessly answered "not authorized — configure the Google Maps API key in global settings" (there is no such in-app setting; the model improvised the guidance). `get_distance_matrix` additionally had NO backend at all. (2) In PlanningTab, the map's `locations` prop bound to the SAVED itinerary stops whenever one existed, so newly added waypoints never rendered while the "Total Waypoints" overlay counted the unsaved array — counter said 3, map showed 2. After reload the editor reset to "No waypoints added" while the map kept rendering the previous draft's stops. The user read their own saved "Route draft 2026-11-21 - Detroit, MI" (Detroit + Cleveland) as mocked data.
- **CAUSE:** The 2026-08-09 hardening made the tools honestly fail but nobody tracked "un-stub once the proxy exists" as work; the page bug came from coupling map rendering to persisted state instead of live editor state.
- **FIX:** `search_places` → existing `findPlaces` callable (query as geocode center, type/query as keyword, zod-validated response); `get_distance_matrix` → NEW `computeDistanceMatrix` callable (zod caps 10x10=100 elements, App Check + auth + secret, `normalizeDistanceMatrix` pure mapper, provider status errors mapped to resource-exhausted/permission-denied/invalid-argument); `get_place_details` stays honestly unavailable (`PLACE_DETAILS_UNAVAILABLE`) with declarations matching in all three agent configs; Road prompt updated (no more "configure an API key" advice). Callable errors normalized (strip `functions/` prefix) into AUTH_REQUIRED / LOCATION_NOT_FOUND / MAPS_PROVIDER_DENIED / MAPS_QUOTA_EXCEEDED. PlanningTab: waypoint editor seeded once from the loaded draft until the user edits it (`waypointsTouchedRef`), map now always mirrors the live list; route-draft DELETE added (TouringService.deleteItinerary + useTouring clears currentItinerary + ConfirmDialog-gated button; rules already allowed owner delete).
- **EVIDENCE:** Renderer: MapsTools 8/8, RoadAgent 12/12, all 18 definition files 96/96, tools dir 362 passed, touring module+services 67/67 (incl. new seed/live-sync/delete tests). Firebase: touring.test.ts 7/7 incl. distance-matrix mapper; firebase `tsc` clean; root `npm run typecheck` clean; eslint clean on every touched file.
- **PREVENTION:** Every fail-closed stub needs a tracked un-stub condition — when a secured proxy ships, grep for tools still stubbed behind it in the same review. "Provider unavailability" answers must never invent user-side remediation (a nonexistent "global settings" field) — the honest failure is the exact blocker plus the real next step. A live-editor UI must render from live state; persisted state is a seed, not the render source.
