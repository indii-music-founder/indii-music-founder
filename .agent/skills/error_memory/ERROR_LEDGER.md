...
- **PREVENTION:** Any feature that executes generated documents inside srcdoc/blob iframes must treat the parent page CSP as its own sandbox ceiling. Two production gotchas confirmed by live probing (2026-09-12): (1) Firebase Hosting header rules are FIRST-match — a later `/creative/**` block never overrides `**`, so path-scoped CSP additions are inert; put required tokens on the `**` block (smallest token possible) instead. (2) Custom headers apply to the REQUEST URL even when the SPA rewrite answers, so `.js` URLs get `immutable, max-age=1y` — an asset URL fetched before its static file existed serves cached index.html for a year. Always ship origin-root assets behind a versioned `?v=` key bumped in lockstep with the file (enforced by `sidecarServing.test.ts`). Before claiming preview works, verify under a meta-CSP replica of the production header (see CSP-replica harness pattern in ISSUE-1433 ledger note), not just on the CSP-less dev server. Related hardening: compiler script-context JSON must use `jsonScriptSafe` (escapes `<`, U+2028, U+2029) — plain `JSON.stringify` lets `</script>` in user strings break out of inline scripts.
