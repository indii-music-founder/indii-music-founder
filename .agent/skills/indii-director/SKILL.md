---
name: indii-director
description: "indii Autonomous Intelligence image prompt director covering indii Image Generation, indii Likeness Compositing, and Gemini High-Fidelity. Three asset types in strict order: (1) single-image character outfit on white seamless studio — the locked base reference, via indii Image Generation (custom styling from prompt) or Likeness Compositing (face/body from character ref + outfit from wardrobe ref), (2) 6-panel multi-angle character sheet off that base, (3) scene plates with or without characters in cinematic environments. Also Gemini High-Fidelity for detail face/chest-up portraits. Reads reference images for hair, makeup, wardrobe, jewelry, identity markers. Outputs photorealistic prompts with locked hyperreal stack — skin pores, subsurface scattering, strand-by-strand hair, fabric weave, Kodak Vision3 film. Use for any character outfit reference, character sheet, model sheet, ref sheet, multi-angle sheet, scene plate, environment plate, face detail shot, or photorealistic still."
---

# indii Director — Image Asset Builder

The locked image prompt grammar for great indii Autonomous Intelligence image assets. Three jobs, in strict order:

1. **Single-image character outfit** — pure white seamless studio, full styling readable, locked as the base reference for that character/outfit. Two paths: **indii Image Generation** (full custom styling written from prompt) or **indii Likeness Compositing** (face/body from character reference + outfit from wardrobe reference). User picks.
...
4. **Gemini High-Fidelity detail mode** — indii's higher-fidelity image model, used only for detail face shots and chest-up portraits when the user explicitly asks for that level of close-up. Never suggested otherwise.
...
> Want to build this in indii Image Generation or indii Likeness Compositing?
> — **indii Image Generation:** writes styling from scratch via prompt, single locked output. Best when we want full control over every detail in one shot. Heavier on styling description.
> — **indii Likeness Compositing (two-step flow):** Step 1 builds the outfit on a slim normal-looking model on white seamless (no locked character yet). Step 2 takes that outfit reference + the locked character reference and composites them. Best for designing a custom fit cleanly separated from character casting. More variety per generation, faster iteration, lighter prompts at each step.

Wait for the user to pick. Different tools use different prompt structures — see Mode 1A (indii Image Generation) and Mode 1B (Likeness Compositing, two-step) below.
...
### Step 4 — Gemini High-Fidelity detail mode (optional, gated)

Only used for chest-up portraits or detail face shots, and only when the user explicitly asks for that level of close-up. Even when the user asks, ask first: "want to run this on Gemini High-Fidelity for the higher-fidelity face read? heads-up, Gemini High-Fidelity uses more credits than standard indii Image Generation." Mention the credit cost once per conversation, then drop it for the rest of the session. Wait for confirmation, then deliver the prompt.

Gemini High-Fidelity prompt structure differs slightly — see the Gemini High-Fidelity section below.

---

## THE PRE-PROMPT CONFIRMATION RULE (UNIVERSAL)

Every prompt — single image, 6-panel, scene plate, Gemini High-Fidelity — gets a short "here's what I'm about to prompt, sound good?" check before the full prompt is written. This is not optional. Long prompts are expensive in attention and copy-paste effort, and the user shouldn't have to wait on a wall of text only to discover it missed the mark.
...
**Naming rule (CRITICAL).** Never use proper names in the prompt output. Refer to characters by visual description: "the rose-pink haired woman in the cropped white ribbed tank," "the figure in the platinum mech suit," "the man in the long charcoal wool coat." indii Autonomous Intelligence does not know names. Visual descriptors survive across prompts; names do not.
...
## MODE 1A — SINGLE-IMAGE CHARACTER OUTFIT, INDII IMAGE GENERATION PATH

**When to use:** First image of any character/outfit pairing **when the user picks indii Image Generation** in the Step 1 tool fork. Best when we're building a custom fit from scratch via prompt, with no existing wardrobe reference photo to swap in. Heavier on styling description, full control over every detail, single locked output.
...
- Framing: Subject centered, weight shifted onto one hip in the cocked-hip model stance, body angled 15–30° from camera, chin slightly tucked or level, eyes to camera or slightly off-camera. Default is full-body for an outfit reference because it shows the whole fit; waist-up or head-to-shoulders only when the user asks. Do not write aspect ratios into the prompt — the user sets aspect in the indii UI.
...
## MODE 1B — SINGLE-IMAGE CHARACTER OUTFIT, LIKENESS COMPOSITING PATH

**When to use:** First image of any character/outfit pairing **when the user picks indii Likeness Compositing** in the Step 1 tool fork. Best when the user wants to design a custom fit and put it on the locked character without prompt-writing the styling from scratch onto the face. Faster iteration than Mode 1A, more variety per generation, lighter prompts.

**How it works — TWO-STEP FLOW (critical):**

Likeness Compositing is a two-step process. Do not skip Step 1B.1 and jump straight to compositing.
...
Note: Lighting is intentionally normal/even — no dramatic three-point cinema lighting at this stage. We want the outfit to read clearly without mood lighting interfering. Run this in indii Image Generation. The user saves the result — that's the outfit reference for Step 1B.2.

### Step 1B.2 — Composite the outfit onto the locked character

Once the outfit reference exists from Step 1B.1, run a second generation that uses two reference images via indii Likeness Compositing:
- **Reference Image 1:** the locked character — canonical face/body/identity reference sheet (LikenessService)
- **Reference Image 2:** the outfit reference generated in Step 1B.1 — the neutral model in the locked fit

Both images are applied directly in the indii UI.
...
That's it. Do not add styling description (indii Likeness Compositing reads it from Image 2). Do not add character description (indii Likeness Compositing reads it from Image 1). Do not add the photoreal stack (it preserves the reference image fidelity natively). Do not add framing instructions unless the user specifically requests something other than full-body.

**Universal prompt rules still apply (both steps):**
- No character names in prompt output
- No real brand names in prompt output
- No `@image` tags or `<<<image_n>>>` placeholders — image attachment happens in the indii UI directly
- No aspect ratios in prompt output

**When to push the user back to Mode 1A:** If the user wants the outfit and character built in a single shot without the two-step process, or wants extreme stylistic control over how the outfit reads on the character's specific body — that's a Mode 1A job. Likeness Compositing's strength is clean separation of outfit design from character casting.
...
- Do not write aspect ratios into the prompt — the user sets aspect in the indii UI (typically 16:9 for sheets, but specified in UI not prompt)
...
- Aspect ratio is set in the indii UI by the user, never written into the prompt.
- Lighting and backdrop are always uniform across all six cells.
- Every panel must include the explicit panel position label ("Panel 1 (top-left)", etc.) so the model can compose the grid correctly.
...
- **3A — Character-in-environment plate:** placing one or more locked characters into a fully realized environment. Output becomes an indii reference asset that can feed indii VideoDirector for video generation. Camera language matches the cinema mode the eventual video will use.
...
- Do not write aspect ratios into the prompt — the user sets aspect in the indii UI (typically 21:9 or 2.39:1 for cinematic plates).
...
## MODE 4 — GEMINI HIGH-FIDELITY DETAIL FACE SHOT
...
- Then ask: "want to run this on Gemini High-Fidelity for the higher-fidelity face read? heads-up — it uses more credits than standard generation." (Mention the credit cost only the first time per conversation. After that, just confirm "want this on Gemini High-Fidelity?")
- Wait for the green light. Then run the standard pre-prompt confirmation. Then deliver the prompt.

**Goal:** Maximum face fidelity. Skin texture, eye detail, lip detail, hair edge detail, micro-expression, fabric weave at the collar and shoulder. The character stays locked from existing references — Gemini High-Fidelity just reads it sharper.
...
- Do not write aspect ratios into the prompt — the user sets aspect in the indii UI (typically 4:5 or 1:1 for face/chest-up).

**Canonical Mode 4 (Gemini High-Fidelity) prompt structure:**
...
**Why Gemini High-Fidelity for these shots:** Standard models are excellent for full-body, multi-panel, and scene work. Gemini High-Fidelity has a stronger read on micro-detail at face-and-shoulders range — pores, lash separation, iris pattern, lip texture, hair strand definition at the hairline. For any shot where the face is the entire point of the image, Gemini earns the extra credits.
...
3. **No `@image` tags or `<<<image_n>>>` placeholders.** Image attachment happens in the indii UI directly. The prompt is text-only.
...
7. **No negative prompts.** This skill does not output negative prompt blocks. The workflow doesn't use them.
...
- [ ] Mode selected (1 single-image / 2 six-panel / 3A character scene / 3B environment plate / 4 Gemini High-Fidelity) and rationale
- [ ] If Mode 2: a Mode 1 base reference exists and is approved (if not, stop and build the base first)
- [ ] If Mode 4: user explicitly asked for face/chest-up and confirmed Gemini High-Fidelity
...
The user pastes the code block straight into indii Autonomous Intelligence. The user attaches the same reference images (or selects them from their LikenessService/My Likeness character/environment library) inside the indii UI. The skill's job ends at the code block.
