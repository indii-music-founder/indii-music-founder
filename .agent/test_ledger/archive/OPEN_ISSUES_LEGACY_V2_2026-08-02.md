...
- **Plan of record (from the integration session, still valid):**
  ...
  - Boardroom validation: `e2e/mcp-boardroom-collaboration.spec.ts` — Publicist delegates artwork to Creative via A2A, Creative executes `queue_release_canvas_render`, synthesized response returns to the Boardroom.

### ISSUE-1093: MCP Tool Suite Expansion (Real Business Logic Integration)

- **Status:** ✅ FIXED (2026-07-20 — P1-P6 + P7a ALL LANDED and verified: register_split_sheet (PDF), draft_cwr_registration (full CWR v2.1), draft_dsp_metadata_xml (fuller DDEX), stage_stripe_payouts (Connect staging), schedule_campaign_waterfall (Inngest), queue_release_canvas_render (ffmpeg canvas; renamed from its historical general render name), audit_sample_clearance (P7a metadata check) are all real backends. **Founder directive 2026-07-20: P7b (fingerprint vendor integration) is a business/vendor-selection action, not an engineering task — it is tracked as a founder action item in `docs/RELEASE_CHECKLIST.md` ("Sample Clearance Fingerprint Vendor") rather than as an open ticket here.** All work an agent can perform without the founder's vendor choice + API key is complete.)
- **Severity:** 🔴 HIGH
...
- **Evidence:**
  ...
  3. Reverse collision: the historical release-canvas tool (now `queue_release_canvas_render`) wrote `status: 'queued'` docs with no `userId`/`prompt`, so `executeVideoJob` fired on MCP rows and marked them `failed` ("Missing required fields") while `processVideoJobs` raced to mark the same doc `completed`. Final state was timing-dependent.
...
- **Module:** `packages/firebase/src/mcp/tools/` — auditSampleClearance, registerSplitSheet, draftCwrRegistration, generatePlaylistPitch, scheduleCampaignWaterfall, stageStripePayouts, queueVideoRender, draftDspMetadata (calculateRecoupment shares the arg-derived pattern but then uid-scopes its queries, which bounds the damage)
...
| `queue_release_canvas_render` | Creative | ✅ real (P6) | ffmpeg canvas MP4 via Inngest, artist's own audio only |
...
- **ISSUE-1222, 1235 → remain 🟡 PARTIAL with narrowed remainders.** Their rules halves are verified live (above). The rules files (`firestore.rules`) have also been manually verified by the agent. `videoJobs` explicitly blocks `create, update, delete` from clients. `users/{userId}` explicitly blocks clients from writing to `tier`, `subscriptionTier`, `plan`, `isFounder`, `entitlements`, `credits`, `creditBalance`, `billing`, etc. What is still unproven is the *authenticated live probe*: for 1222, that a real verified Free user cannot alter an authority field and that backend provisioning can still grant Founder/paid entitlements; for 1235, that a normal callable returns a server job ID + reservation and that a rejected request creates neither a job nor Vertex work. Both need a real signed-in session.
- **NOTE (2026-08-01):** The user manually fixed a typo in `videoEditorStore.ts` (`proxy.name` to `proxy.path`) to fix a hydration path error. Typecheck is currently running to verify.
...
- **Status:** ✅ FIXED (2026-08-02 — Per explicit founder directive, deleted dead legacy WS/Ngrok mobile remote fallback code (`mobile_remote.ts`, `IndiiRemoteService.ts`, `launch_remote.ts`, `RemoteTransportPolicy.ts`) and removed orphaned IPC type declarations in `electron.d.ts`. Documented that Mobile Remote uses the authenticated Firestore cloud relay (`RemoteRelayService.ts` / `RemoteSection.tsx`) exclusively.)
- **Resolution:** Deleted dead legacy files (`mobile_remote.ts`, `IndiiRemoteService.ts`, `launch_remote.ts`, `RemoteTransportPolicy.ts`), cleaned up `main.ts` startup and shutdown hooks, and pruned orphaned type declarations in `electron.d.ts`. Verified full workspace `npm run typecheck` clean (0 errors).
...
- **Verification completed:** Sentry MCP connection was restored. Triggered intentional `Test crash for ISSUE-1296 (Global Unhandled Error)` and `Test crash for ISSUE-1296 (Unhandled Promise Rejection)` using `trigger_crash.cjs`. Confirmed via Sentry MCP that both issues appeared under `thewalkingagency/indii-music-founder` project on the Sentry Dashboard.
- **Acceptance completed:** Sentry Dashboard verified to receive events successfully. No test events were unintentionally filtered by `beforeSend` or `ignoreErrors`. Sentry capture integration is fully verified.
