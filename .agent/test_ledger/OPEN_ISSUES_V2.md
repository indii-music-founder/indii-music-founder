...
- **Verification:** `npm run typecheck` clean (all 4 workspaces + firebase test gate); `npm run build -w packages/firebase` succeeds and produces `packages/firebase/lib/index.js`. No unit test exists or is warranted for a numeric memory-tier config value; verification here is the production log evidence itself (root cause identified) plus the next live deploy's Cloud Functions step passing (to be confirmed after push).
