...
- **Status:** 🟡 PARTIAL — verified enrollment is deployed; administrator invitation and consent-respecting milestone delivery are implemented with local structural proof; genuine delivery/acceptance and legacy promotion remain open
- **Severity:** 🟠 HIGH
...
- **Evidence:** The landing sends a Firebase passwordless email link and only calls `joinFoundingArtistWaitlist` after the SDK returns an email-verified user. The callable derives email and UID from the verified token, assigns immutable queue order in a transaction, deduplicates through a SHA-256 email index, records consent plus a canonical enrollment event, and writes `foundingArtistWaitlist/{uid}`. Existing artists can use the verified-email preference path linked from every milestone email to disable or restore milestone updates without changing queue position; each preference change is recorded as a lifecycle event. Firestore rules deny direct client access to all canonical waitlist/index/meta/event collections. The authenticated Founders Dashboard merges that verified queue with deduplicated legacy `waitlist` submissions and labels their different trust states. The administrator communication implementation queries the earliest `waitlisted` record, queues fixed beta copy, marks `invited` only after Resend succeeds, and records failures without consuming the artist's place. Major-milestone campaigns include only verified/active/opted-in artists, recheck consent before sending, escape administrator text, use deterministic provider idempotency keys, and record per-recipient sent/skipped/failed outcomes. Dashboard actions require explicit confirmation. Focused Firebase/admin/landing tests, scoped lint, and affected production builds pass locally. These are structural proofs, not evidence that a genuine artist received, opened, or accepted a production message.
- **Impact:** Fake emails, duplicate records, and client-controlled priority would undermine beta access.
- **Remaining:** Confirm Firebase Email Link authentication, authorized domains, and App Check configuration with one genuine-user enrollment; deliver and inspect one real beta invitation plus one opted-in milestone update; implement invitation acceptance/access activation; deploy or verify the updated Founders Dashboard surface; migrate or resolve legacy unverified records.
...
### ISSUE-1431: Creative artist journey needs verified phone-to-social and song-video delivery

- **Status:** 🟡 PARTIAL — local implementation; release acceptance blocked
- **Severity:** 🟠 HIGH
- **Module:** Creative Department / Omni Flash / session ingestion / editor / render queue
- **Source of truth:** Founder request: simplify artist workflows using existing Google APIs, retain iPhone originals and speech, allow independent social clips, configurable song duration, and optional named likeness references. No new provider adapter.
- **Changes:** Journey navigation and direct Omni Flash entry; phone MIME normalization; scoped resumable-upload lifecycle; inspected proxy duration/audio/source metadata; source-aware splits; separate editable social timelines in original/9:16/4:5 framing; 180-second song guidance and configurable 420-second maximum; named optional Omni likeness references; save-before-cloud-export; queued server-owned timeline snapshots; account/project-safe render history and preview.
- **Limits:** Trimmed animations and approved synced sections require rendering before social clipping. Portrait copies fit the complete picture; the artist can reframe the copy. Likeness must be reviewed. Long uploads retain the existing 20-GiB service limit. Social publishing/account integrations are not claimed verified.
- **Evidence (2026-09-09):** Restored checkout based on `dc612b3f9`; 50 targeted suites / 251 checks passed. These are pure/legacy structural checks, not authenticated customer-path proof. Production build stopped in `preflight:prod`: required Firebase/App Check/ingestion settings and service credentials are absent. No environment or credential values were invented or changed. The last actual browser navigation to `https://indii.music/` showed Firebase “Site Not Found”; no sign-in, account creation, tier verification, uploads, generation, or publishing occurred.
- **Delivery:** The founder explicitly requested committing and integrating all changes into the correct repository after the production-configuration limitation was disclosed. This records authorization to deliver the reviewed code to main separately from deployment acceptance. Full typecheck and lint passed; 251 targeted checks passed. Unified CI has no complete verdict after interrupted runs; exact-SHA remote CI must be reported separately. Production build and genuine artist acceptance remain unverified; no deployment success is claimed.
- **Acceptance:** Genuine artist signs in at the approved live domain; imports/resumes a long iPhone recording; reviews orientation/color/audio; trims and exports; creates independent social clips before/after music-video creation; uses a finished song/photos/footage within the configurable duration ceiling; optionally selects named band-member references and reviews resulting likenesses; refreshes/switches projects without lost or cross-account work; downloads the real completed MP4. Verify the social-post handoff and production rendering with the real configured services.

### ISSUE-1432: Timeline NLE overhaul (c6a4c3035) verified — solo is cosmetic, track reorder has no UI, ⌘⌫ deletes clips while typing in inputs

- **Status:** 🟢 RESOLVED (2026-09-11) — all three gaps + minor hygiene fixed in one commit; compiler-output + unit verification only, no live-user claim
- **Fix evidence (2026-09-11):** (1) Solo: `CompiledTrack.audible = !isMuted && (!anySolo || isSolo)` in `packages/video-compiler/src/compiler.ts` — applied to `data-volume` AND audioFade tweens (fade tweens now skipped for inaudible tracks, closing a pre-existing mute leak where faded clips on muted tracks played audio); `isSolo` added to the `track-controls-require-composition` gate in `packages/shared/src/types/videoRoute.ts` so the direct-media fast path cannot bypass solo. (2) Reorder: ChevronUp/Down buttons in `TimelineTrack` header wired to `moveTrack`, disabled at edges and when locked. (3) ⌘⌫/⌘Z/⌘⇧Z guarded against INPUT/TEXTAREA/contentEditable targets in `VideoEditor.tsx`. Minor: monotonic seek-nonce counter, `VideoPreview` duplicate seek effect removed, outer drop handler zoom-aware, duplicate `min-h` classes removed. Validation: 59 files / 432 tests green (video tree + compiler + videoRoute), typecheck exit 0, lint 0 errors (176 warnings = baseline).
- **Severity:** 🟠 HIGH (solo no-op + accidental destructive shortcut) / rest 🟡 MEDIUM–LOW
- **Module:** `packages/renderer/src/modules/creative/video/editor/` + `store/videoEditorStore.ts`
- **Verified (independent re-run, 2026-09-11):** editor+store suites all green (29 files / 385 tests incl. wider video dir); `npm run typecheck` exit 0 across all packages; `npm run lint` 0 errors; exact-SHA CI run 34608452402 `completed success`; commit is ancestor of origin/main. Store transient-drag→single-undo-commit, dual-edge snapping, split/duplicate/keyframe partitioning, µs-window waveforms, 200px offset geometry, and ARIA ruler all implemented and honestly tested.
- **Gap 1 — Solo is a no-op:** `toggleSoloTrack` flips `isSolo`, but no compiler/preview path reads it (grep: only UI + tests). `isMuted` IS honored (`packages/video-compiler/src/compiler.ts:147`). Soloing a track silences nothing.
- **Gap 2 — Track reorder unreachable:** `reorderTracks`/`moveTrack` are store-only, well-tested, but no component calls them; R3's "reorder" control has no UI affordance.
- **Gap 3 — Global ⌘⌫ ripple-delete fires in text fields:** `VideoEditor.tsx` keydown handler never checks `event.target` for input/textarea/contentEditable, so the standard macOS "delete to line start" shortcut inside a clip-name/property field ripple-deletes the selected clip (undoable, but destructive surprise).
...
- **Status:** 🟢 RESOLVED (all six gaps fixed & verified; 60 files / 447 tests green, typecheck 0, lint 0 errors)
...
- **Gap 4 — RESOLVED (commit 589c9cce9):** clip move, both trim handles, and keyframe diamonds bind `onPointerDown`; drag hooks listen on `pointermove`/`pointerup` with `pointercancel` as an abort path (same pre-drag restore as Escape). `setup.ts` now provides a spec-shaped `PointerEvent` polyfill (clientX/clientY/button/pointerId) — jsdom's absence of it made `fireEvent.pointer*` coordinate-less (NaN deltas); the polyfill is keyed so local test polyfills (TimeRuler) keep precedence. Adversarial high-frequency drag stress converted to pointer dispatches. Mouse behavior unchanged; touch/pen gain support.
...
- **Gap 6 — RESOLVED (commit 589c9cce9):** `commitTransientClipUpdate` diff widened with `name`, `volume`, `opacity`, `scale`, `playbackRate`, `keyframes` (reference compare) — value-only drag commits now record their single undo snapshot. Verified by `useVideoEditor.undoRedo.test.ts` (volume-only and name-only commits each yield exactly one past entry).
- **Delivery note (honest history):** commit `8407bed83` ("NLE keyboard transport") accidentally carried every staged file in the shared index — this session's Phase 2 (nearest-track drops) AND the parallel agent's Gap 1/3 fixes plus its Electron IPC sweep landed under that one message. Content passed the pre-commit gates and CI; no history rewrite per mainline policy. Commit `589c9cce9` is strictly path-limited to this session's Phase 3+4 files.
- **Also noted — RESOLVED (browser live compiled preview, 2026-09-12):** the compiler is pure TS and now compiles in-browser via the `PlatformBridgeService.compileVideoPreview` web fallback (`packages/renderer/src/services/video/webPreviewCompiler.ts`). Serving the `gsap.min.js` sidecar + HyperFrames runtime to the web player required its own investigation, findings: (a) `@hyperframes-player` runs compositions in a sandboxed srcdoc iframe that INHERITS the app's CSP — production `/creative/**` `script-src` has no `unsafe-inline`, so the compiler's inline GSAP plan was silently dead on web; (b) the player auto-injects its runtime from jsDelivr when it sees timelines without a bridge — blocked by prod CSP and unnecessary; (c) `./gsap.min.js` in srcdoc resolves against the parent SPA route, not origin root. Fix: sidecar (`public/gsap.min.js`) + pinned runtime (`public/hyperframe.runtime.iife.js`, byte-identical to the `@hyperframes/core` dist the installed player pins) served same-origin; transform pins the sidecar src to `/gsap.min.js`, pre-injects the runtime (player's own dedupe skips jsDelivr), and moves the byte-identical GSAP plan into a same-origin blob script. `firebase.json` adds `blob:` to `script-src` for `/creative` + `/creative/**` ONLY (landing and app-wide `**` stay strict). Desktop IPC path untouched. Security hardening found en route: compiler script-context JSON now escapes `<`/U+2028/U+2029 (`jsonScriptSafe`) so `</script>` in countUp prefix/suffix or keyframe-sourced strings can no longer break out of the timeline script (latent on desktop, live same-origin injection once web preview shipped). Drift guard: `sidecarServing.test.ts` byte-compares all three sidecar copies + the runtime pin. Validation: 405 video-tree tests + 32 compiler tests + 17 bridge/transform tests green; full typecheck 0; CSP-replica browser harness (real Chromium, exact `/creative` script-src, no inline/CDN allowances) PASSED — transform checks, timeline time advanced on play, `gsap=true timelines=true runtimeBridge=true`, zero `securitypolicyviolation` events. In-app live validation with a real signed-in browser artist still pending (REAL_USER_AUTHENTICITY — not claimed here). Production deploy verification (same day, corrected): the studio app target lives at **`app.indii.music`** (site `indii-music-studio`); `indii.music` and `founder.indii.music` are the LANDING site and serve landing HTML + landing headers for every non-asset path — probing them produced two false conclusions, retracted here: (a) "edge-poisoned sidecar URLs" (false — the app target serves real JS at versioned AND unversioned URLs: 73135B / 392445B, text/javascript); (b) "Firebase header precedence is first-match / creative blocks dead" (false — `app.indii.music/creative/video` serves the `/creative/**` block: COOP `same-origin` + scoped `blob:` in script-src, exactly the intended design). What shipped and stays: scoped `blob:` on `/creative` + `/creative/**` only (verified live on the studio origin), and versioned sidecar cache keys (`/gsap.min.js?v=3.14.2`, `/hyperframe.runtime.iife.js?v=0.8.11`, lockstep-guarded in `sidecarServing.test.ts`) because production serves .js URLs `immutable, max-age=1y` — versioned keys are how a future sidecar update reaches returning visitors. LESSON: verify the custom-domain → hosting-site mapping (`.firebaserc`) BEFORE diagnosing production header/serving anomalies. POP-OUT FALLBACK + JEV GUIDANCE (unit closed 2026-09, delivered inside parallel commit `b548d67d1` — honest history, same carried-commit pattern as `8407bed83`): (1) ScreenControl 'Pop Out Viewer' previously SILENTLY did nothing outside Chromium (Window Management API missing/denied → logger.error + return). Now `openProjectorWindow` returns 'projector' | 'popup' | 'blocked': a centered same-origin popup fallback (noopener — viewer syncs over BroadcastChannel, never needed opener), with `canProjectToSecondScreen()` exposed; five unit tests cover fallback/placement/no-screens/blocked/non-HTTP. (2) Jev judgment 6 (`judgePreviewErrorGuidance`, Choice: trim_timeline/fix_media/retry/none) translates failed-preview compiler errors into artist-facing guidance in the pop-out error state; flag-gated via TYPESAFE_JUDGMENTS, null-fallback keeps raw error — the judgment only ever adds guidance. All guided through the existing server-side `typesafeJudge` callable + `typesafeJudgments.ts` review contract. Component contract locked by `VideoPopout.guidance.test.tsx` (raw error always visible; guidance only adds; stale guidance drops on re-error; artifact fallback + live-player rendering). Verified at tip state: judgments+screen suites 190 green, video tree 58 files/419 green, typecheck 0, CI 36192500432 success. TYPOGRAPHY GAP CLOSED (same day): the compiler's 8 embedded families had NO font files anywhere — previews AND renders rendered text as fallback sans-serif (the earlier claim "final render embeds fonts properly" was false and is retracted). Now: `scripts/vendor-video-fonts.mjs` vendors latin-subset OFL woff2 snapshots (8 families, 195KB base64) into `packages/video-compiler/src/fontAssets.generated.ts`; `compileProjectToHyperFrames(project, { fontAssets })` emits `@font-face` data-URIs for exactly the used families/weights (nearest-weight deterministic match); all three executors inject them (web bridge lazy-import, desktop main, render-worker) so preview and render share one typography path. CSP: `data:` added to font-src on app `**` + `/creative` blocks (landing untouched). Verified by compiler/transform suites + CSP-replica Chromium harness (`document.fonts.check` true inside the srcdoc for both a static and a variable family, zero violations). Sidecar upgrade friction removed: `npm run sync:video-sidecars` re-syncs gsap/runtime copies from their canonical sources and bumps the `?v=` cache keys; lockstep suite remains the verifier.
...
- **Evidence:**
  ...
  2. Cloud Build log `bf885663-112e-4da8-892e-f9d2adfb9ded` isolated the failure to Google's Node 22 buildpack step: `Running "npm install --package-lock-only --quiet"`.
...
- **Status:** 🟡 PARTIAL (2026-09-22 — module-level sync was already working (see Correction); the real gap, tab-level deep links, is now fixed. Locally-owned `useState` tabs remain out of scope by design)
- **Severity:** 🟡 MEDIUM
...
- **Fix (2026-09-22, tab deep links):** `hooks/useURLSync.ts` gained a `TAB_STATE` config for store-backed tabs (`distributionTab` default `releases`, `financeTab` default `overview`). URL→Store: `?tab=` is parsed on every route change (including Back/Forward) and applied via the module's store-backed tab setter; an explicit `?tab=` wins over the legacy alias tab (`/audio-analyzer`→qc). Store→URL: module switches carry a non-default active tab onto the new URL; same-module tab switches mirror into `?tab=` with `replace` (no history spam); navigating to a module without URL-addressable tabs strips a stale param. An explicit `?tab=` now round-trips end-to-end: `/finance?tab=royalties` restores the Royalties tab after reload, Back/Forward, or share.
- **Evidence:** `packages/renderer/src/hooks/useURLSync.ts:44-70` (`TAB_STATE` config + `buildUrl`), `:106-137` (URL→Store tab application), `:151-184` (Store→URL carry + mirror + stale-param strip), `packages/renderer/src/hooks/useURLSync.test.ts` (14 tests incl. the 5 new ISSUE-1439 cases).
- **Files:** `packages/renderer/src/hooks/useURLSync.ts`, `packages/renderer/src/hooks/useURLSync.test.ts`
- **Verification:** typecheck 0 errors; eslint 0 errors on touched files; useURLSync suite 14/14 passing.
...
- **Status:** ✅ FIXED (2026-09-23 — commits `19a299396`, `68e6722e8`, `7a229800b`, `7773c343e` + residue pass: OmniWorkflow, ReleaseWizard, QCPanel, Finance/Marketing grouping, DirectGenerationTab pills, MerchDesigner gating/swatches, MemoryDashboard filters, StudioControlsPanel governance reorder + budget-ledger popover — all landed. See the final residue-pass Fix line below)
- **Severity:** 🟠 HIGH
...
- **Fix (partial 3, 2026-09-22 — QCPanel):** Metadata intake now shows only the two Run-QC-required fields (Title, Artist) plus action buttons; ISRC/Artwork URL/UPC moved into a collapsed "Optional & Content ID fields" SectionCard; the Content ID Rights Attestation moved into its own collapsed SectionCard that auto-opens when CID generation fails validation, so failing fields are where the user is looking. Both sub-tab panes are conditionally mounted (were always-mounted with a `hidden` class). Audit's "merge the two handoff buttons" recommendation was examined and REJECTED: the two buttons send different payloads (video prompt vs image prompt) — merging would hide the image path, not deduplicate anything. Metadata tab visible controls 13→~7.
- **Evidence (QCPanel):** `packages/renderer/src/modules/distribution/components/QCPanel.tsx:290-294` (disclosure state + auto-open comment), `:371-375` (setRightsOpen(true) in CID catch), `:733-800` (required fields + Optional section), `:802-846` (Rights Attestation SectionCard), `:428-433` + `:721-726` (conditional pane mount), `packages/renderer/src/modules/distribution/components/__tests__/QCPanel.test.tsx` (updated navigation helpers; 8/8 passing incl. new auto-open assertion).
- **Files (QCPanel pass):** `packages/renderer/src/modules/distribution/components/QCPanel.tsx`, `packages/renderer/src/modules/distribution/components/__tests__/QCPanel.test.tsx`
- **Fix (partial 4, 2026-09-22 — Finance + Marketing grouping):** Finance's 16-tab strip (which needed `overflow-x-auto` to fit) is now 5 group triggers (Overview / Spending / Royalties / Compliance & Tools / Merch) with a pill sub-strip inside the active group. Tab values, panels, `data-testid="finance-tab-*"` hooks, and `?tab=` deep links are unchanged — only trigger chrome was grouped, so `/finance?tab=forensics` still restores the exact panel (the active group derives from `currentTab`). Marketing's rail regrouped from 2 sections of 14 flat tabs into 5 scannable sections (Overview / Release Assets / Channels / Paid & Partners / Intelligence) with tab ids unchanged; stub hiding from the ISSUE-1436 partial remains in place.
- **Evidence (grouping):** `packages/renderer/src/modules/finance/FinanceDashboard.tsx:73-121` (`TAB_GROUPS` + `activeGroup` derivation), `:169-198` (group triggers + sub-strip; `finance-tab-*` testids preserved), `packages/renderer/src/modules/marketing/components/MarketingSidebar.tsx:34-77` (`navSections` regrouping). `FinanceDashboard.test.tsx:53-71` updated to assert the grouped structure (3/3), Marketing suite 1/1.
- **Files (grouping pass):** `packages/renderer/src/modules/finance/FinanceDashboard.tsx`, `packages/renderer/src/modules/finance/FinanceDashboard.test.tsx`, `packages/renderer/src/modules/marketing/components/MarketingSidebar.tsx`
- **Tier 3 dispositions (2026-09-23, founder-delegated, Jev-informed):** All four orphan/overlap calls RESOLVED as keep — analytics standalone (jev 0.87: genuinely distinct depth vs Home widgets), crm standalone (0.33 unclear — conservative keep; revisit on real usage), screenwriter kept + "Open in Screenwriter" jump link added from Creative video storyboards (`StoryboardTimeline.tsx`, 0.76: distinct workflow worth a destination), capture kept as gated deep-link target (0.36 unclear — conservative keep, it is dev-gated). Release-journey flow: not built — remains a future product decision. Evidence: batched jev judgment over the real module sources (6.4k tokens), `.agent/AGENT_BRIDGE.md`.
- **Fix (final residue pass, 2026-09-23):** DirectGenerationTab — 8 quick-modifier pills collapsed into a "+ Modifiers" popover (append-only ratchet action); Camera Movement + Motion Strength demoted from Basics to Advanced. MerchDesigner — align tools disabled until 2+ fabric objects are selected (reactive selection:created/updated/cleared state; they rendered usable while being runtime no-ops below the threshold), 6-swatch dock collapsed into a toggleable panel anchored on the color trigger. MemoryDashboard — category/tier filters collapsed under a Filters disclosure. StudioControlsPanel — governance cards demoted below the core creation flow; budget ledger collapsed behind a "Daily creative budget" chip popover; full metered-operations ledger moved to the History tab (Refresh/Load-more fully usable). Group assignments via TypeSafe jev grouping judgments over deterministic grep control inventories (3 batched calls, ~8k tokens). Suites: merch 62/62, memory+creative+right-panel+judgments green; typecheck 0; eslint 0. Cross-ref: `.agent/AGENT_BRIDGE.md`.
- **Fix (final residue pass, 2026-09-23):** DirectGenerationTab — 8 quick-modifier pills collapsed into a "+ Modifiers" popover (append-only ratchet action); Camera Movement + Motion Strength demoted from Basics to Advanced. MerchDesigner — align tools disabled until 2+ fabric objects are selected (reactive selection:created/updated/cleared state; they rendered usable while being runtime no-ops below the threshold), 6-swatch dock collapsed into a toggleable panel anchored on the color trigger. MemoryDashboard — category/tier filters collapsed under a Filters disclosure. Group assignments made via TypeSafe jev grouping judgments over deterministic grep control inventories (3 batched calls, ~8k tokens). Suites: merch 62/62, memory+creative+right-panel green; typecheck 0; eslint 0. Cross-ref: `.agent/AGENT_BRIDGE.md` coordination + shadow artifacts `typesafe-shadow-experiments.{md,json}`.

### ISSUE-1441: Dead and duplicated UI inventory (dead buttons, twin components, orphaned layout code)

- **Status:** ✅ FIXED (2026-09-23 — commits `7148402f0`, `0632d5cb3` + residue pass: dead controls removed, deprecated revenue-widget duplicates unregistered, twins deduped, FounderReadiness deduped, social+publishing on ThreePanelDashboard, `MobileAdaptiveLayout` deleted, governance cards demoted, MerchDesigner/DirectGenerationTab/MemoryDashboard density passes landed)
- **Severity:** 🟡 MEDIUM
...
- **Fix (2, 2026-09-22 — ThreePanelDashboard actions slot + social/publishing migration):** `ThreePanelDashboard` gained an optional `actions?: ReactNode` slot rendered right-aligned in the standard header — the missing slot that forced social and publishing to hand-copy the header/aside markup (~80 duplicated lines). `SocialDashboard` and `PublishingDashboard` now compose `ThreePanelDashboard` (leftPanel/rightPanel/actions props) instead of their own absolute-positioned 3-column shells, so both inherit AdaptiveWorkspace drawer rails on narrow viewports (previously their rails silently vanished — `hidden @5xl:flex` with no fallback) plus the standard header/testid contract. Publishing's clickable-title-to-dashboard behavior is intentionally dropped (the sidebar "Return to HQ" covers it); its New Release / Add Account / Create Post header buttons are preserved verbatim via the actions slot.
- **Evidence (migration):** `components/layout/ThreePanelDashboard.tsx:33-38,63-65` (actions prop + header render), `modules/social/SocialDashboard.tsx:111-129` (ThreePanel composition), `modules/publishing/PublishingDashboard.tsx:113-148` (ThreePanel composition). Suites: social 39/39 (9 files), publishing 42/42 (6 files), finance+legal+licensing+devops+screenwriter consumers 79/79 (19 files). typecheck 0 errors on touched files; eslint 0 errors.
- **Files (migration pass):** `packages/renderer/src/components/layout/ThreePanelDashboard.tsx`, `packages/renderer/src/modules/social/SocialDashboard.tsx`, `packages/renderer/src/modules/publishing/PublishingDashboard.tsx`
...
- **Status:** ✅ FIXED (2026-09-22 — all seven defects fixed across two commits)
...
- **Fix (partial, 2026-09-22 — foundry cluster):** New `packages/shared/src/foundry/parseMoney.ts` — deterministic statement-money parser (US/EU/auto, accounting parentheses, currency-symbol stripping, null-on-unparseable). Both TuneCore and both DistroKid adapters (renderer + shared copies) now parse earnings through it and route null into the existing quarantine path instead of booking corrupted magnitudes; TuneCore's fabricated `'US'` territory default replaced with an empty value; both adapters capture a real fee column (`fee`/`distributor fee`/`commission`, default 0 only when absent) so LayeredValidator's gross=net+fee reconciliation receives real inputs; `inferSemantic` checks the fee branch before the currency branch in both FormatForensicsEngine copies, fixing the "Fee Amount" misclassification.
- **Evidence (foundry cluster):** `packages/shared/src/foundry/parseMoney.ts` (parser + documented rules), `packages/shared/src/foundry/parseMoney.test.ts` (6 tests: US/EU round-trip, parentheses negatives, auto-detection, format hints, null-on-garbage), `packages/renderer/src/services/foundry/adapters/TuneCoreStatementAdapter.ts:66` (`parseMoneyAmount(rawEarnings)` + null quarantine), `:76` (empty country, no fabrication), `packages/renderer/src/services/foundry/adapters/DistroKidStatementAdapter.ts:83` (same treatment; DistroKid had no country default — Jev-verified), both FormatForensicsEngine copies (`must precede the earnings branch` comment at the fee check). Shared package rebuilt; foundry suites 57/57 (9 files); typecheck 0 errors; eslint 0 errors.
- **Fix (2, 2026-09-22 — severity, moods, sensitivity):**
  1. Severity now derived from the CONTRACT text (`/training|perpetual|...|voice clone|soundalike|digital double/i.test(text)`) instead of the canned flag strings; added the three missed needles ('voice clone', 'soundalike', 'digital double'). Known deterministic limitation, documented in-code and in tests: negated grants ('no sublicensing permitted') still rate HIGH — negation semantics is the TypeSafe Noul candidate.
  2. `mapToSyncMoods`: substring matching replaced with exact-lookup + whole-word regex (kills 'unhappy'→Upbeat, 'lovelorn'→Romantic; compound tags like 'dark-pop' still match); the fabricated `'Chill'` fallback replaced with `[]` — unrecognized moods write nothing instead of poisoning the sync matcher.
  3. Evidence sensitivity: USD/Earnings-only test replaced with a currency- and wording-agnostic indicator list ('revenue', 'royalt', 'payout', 'net receipts', 'eur', 'gbp', …) plus a decimal-amount pattern — non-USD statements now classify sensitive_financial and keep snippet masking.
- **Evidence (pass 2):** `CreatorProtectionHarnessService.ts` (`highRisk` tests `text`; 3 new needles; ISSUE-1443 comment), `SyncMetadataTaggingService.ts` (whole-word matching + no-fabricated-fallback comment), `EvidenceIntakeService.ts:70-79` (indicator list + amount pattern). Tests: `SyncMetadataTaggingService.test.ts` (unhappy/lovelorn not mapped; [] fallback; compound tags still match), `CreatorProtectionHarnessService.test.ts` (HIGH from contract text; flagged-but-benign stays MEDIUM; missed phrasing caught), `EvidenceIntakeService.test.ts` (new, 4 tests: EUR/GBP statements sensitive; benign stays confidential; override honoured). Suites: 11/11 + 4/4; typecheck 0 errors; eslint 0 errors.
- **Files (pass 2):** `packages/renderer/src/services/creator-protection/CreatorProtectionHarnessService.ts` + test, `packages/renderer/src/services/licensing/SyncMetadataTaggingService.ts` + test, `packages/renderer/src/services/foundry/EvidenceIntakeService.ts`, `packages/renderer/src/services/foundry/__tests__/EvidenceIntakeService.test.ts` (new). `packages/shared/src/foundry/parseMoney.ts` (new), `packages/shared/src/foundry/parseMoney.test.ts` (new), `packages/shared/src/foundry/index.ts`, both `adapters/TuneCoreStatementAdapter.ts`, both `adapters/DistroKidStatementAdapter.ts`, both `FormatForensicsEngine.ts` (renderer + shared copies of each).- **Delivery addendum (2026-09-23 /end):** Stage 2A knowledge fold `b7274eb44`; Stage 2B marketing fold `aa0b26527` (+ test alignment `dffac1175`, `1e8b98284`); Stage 3 backend pruning `95b10e787` (+ resurrect fix `7de27c39c`); format-foundry rename completion folded into Stage 3 follow-up. **TypeSafe rollout (4 judgment points, all via server-side `typesafeJudge` callable — no key material in renderer, no renderer-side direct API calls):** pilot `0367d5071` (transient-error Noul), guardrails `dca5f9dfb` (injection hazard Nouls — regex-flagged inputs only, static-critical never probabilistically cleared), default-ON + failure cooldown `6dbf0c94a`, memory composite `bd8437399` (replaces the always-fired TEXT_FAST LLM re-rate). Mode: live-ready, **currently deterministic baselines answer** until TYPESAFE_API_KEY is provisioned in GCP/CI (proxy returns failed-precondition; cooldown suppresses retry spam). Observed agreement: unit-test level only (bands, fallbacks, escalation policy) — production outcome evaluation pending; sibling shadow-mode experiments (`b7cba39db`, foundry columns) are the evaluation vehicle. Token cost: 1 proxy call per judgment event (memory composite batches 3 questions into 1 call; injection only fires on regex-flagged input; skill intent only on deterministic miss). Coordination channel: `.agent/AGENT_BRIDGE.md`.
- **Founder decision (2026-09-23, "keep both"):** `createMicroTransaction` and `activateFounderPass` callables are RETAINED. Both remain unwired-by-design until a product decision builds their surfaces; they are NOT deletion candidates anymore. Also this date: production hosting refreshed from a clean `origin/main` checkout (stale CI artifact chain was serving a pre-Stage-2 bundle); Marketing tab strip + Knowledge tabs verified in the deployed production chunks (`MarketingDashboard-CCqeY9OT.js` contains the tab strip and BrandManager wiring). TYPESAFE_API_KEY provisioned (SM v1) — typesafeJudge recreates on the next functions deploy, judgments live-by-default with deterministic fallback.

### ISSUE-1444: assetVersions Firestore subcollection had no security rule — every artifact finalization failed with permission-denied (and agents confabulated a "DevOps escalation")

- **Status:** ✅ FIXED (2026-09-26 — rule + 5 emulator tests + agent-honesty hardening; rules deploy pending owner trigger)
- **Severity:** 🔴 CRITICAL (Artifacts feature completely non-functional in production: `record_asset_version` / `promote_asset_version` and every tool-side `AssetVersionService.recordVersion` call failed)
- **Module:** Creative finalization / Firestore security rules / agent honesty
- **Evidence (repro):** Creative Director chat: `record_asset_version` → "permissions error when trying to write to the asset registry". `AssetVersionService.recordVersion` writes `users/{uid}/assetVersions/{assetId}/versions/{versionId}` (`AssetVersionService.ts:80,106`); `packages/firebase/firestore.rules` contained zero matches for `assetVersions`, so every write fell through to the final catch-all deny. `promoteVersion` additionally fails on its `getVersionTree` read. Artifacts UI empty because nothing was ever persisted. Root-caused 2026-09-26 by founder-requested investigation; full chain in the session log and ERROR_LEDGER (`ASSET_VERSIONS_MISSING_FIRESTORE_RULE`).
- **Impact:** No artist could ever finalize an asset into an artifact. Secondary trust damage: indii Conductor fabricated an escalation "to DevOps via the direct-to-dev pipeline" — no such pipeline exists in the codebase (no tool, queue, function, or record); the story was pure confabulation covering a tool failure.
- **Fix:**
  1. `firestore.rules`: added `match /users/{userId}/assetVersions/{assetId}/versions/{versionId} { allow read, write: if isOwner(userId); }` (mirrors LikenessService per `docs/CREATIVE_FINALIZATION_TOOLS_PLAN.md` §13).
  2. `firestore.rules.test.ts`: 5 emulator tests — owner record version / read tree allowed; other-user write + read denied; unauthenticated write denied.
  3. Agent honesty hardening in BOTH execution-contract copies (renderer `AgentPromptBuilder.ts` + relay `agentPrompts.ts` per SYNC NOTE): TOOL-FAILURE HONESTY (quote raw tool errors verbatim; never reframe failure as success) and NO PHANTOM ESCALATIONS (only claim escalations/handoffs a tool actually performed and evidenced).
- **Acceptance:** Emulator rules suite green (257 passed incl. the new block; targeted run 5 passed / 249 skipped with denial evidence at the new rule line). Remaining: owner runs `firebase deploy --only firestore:rules` to production, then live-verifies `record_asset_version` → artifact visible on the Artifacts page (real-user verification per `.agent/REAL_USER_AUTHENTICITY.md`).
- **Files:** `packages/firebase/firestore.rules`, `packages/firebase/src/test/security/firestore.rules.test.ts`, `packages/renderer/src/services/agent/builders/AgentPromptBuilder.ts`, `packages/firebase/src/relay/agentPrompts.ts`, `.agent/skills/error_memory/ERROR_LEDGER.md`
...
- **Status:** ✅ FIXED (2026-09-26 — commit `c8f4e29b2` on main; CI run 36210621617 green incl. deploy-production; **served production bytes verified**)
...
- **Root cause:** Commit `01a72211a` (2026-09-22, ISSUE-1442 registry cleanup) wired `[FEATURE_FLAG_NAMES.PROJECT_CANVAS]: ['project-canvas']` into `GATED_MODULES` (`packages/renderer/src/config/featureFlags.ts:131`). Before that commit the flag existed but was never mapped, so Project Canvas shipped to production regardless (the commit's own comment admits this). The flag default is `import.meta.env.DEV || import.meta.env.VITE_ENABLE_PROJECT_CANVAS === 'true'` — false in production builds; `VITE_ENABLE_PROJECT_CANVAS` is set nowhere (not in `.env`, `.env.example`, or `.github/workflows/deploy.yml`), and no Remote Config override has been verified (firebase CLI reauth needed to check). Effective production value: gated.
- **Impact:** Founder loses the production Project Canvas they had been using since it shipped. Module code is fully intact (`packages/renderer/src/modules/project-canvas/` untouched); this is purely a gate regression, not data or code loss.
...
- **Delivery evidence (2026-09-26):** Fix commit `c8f4e29b2` delivered `HEAD:main` per mainline refspec. Its solo CI run was cancelled by the concurrent `6bc7cd848` push; successor run **36210621617** (tree contains the fix) completed **success** through deploy-production. **Served-bytes verification:** crawled the deployed Studio app (site `indii-music-studio.web.app` per `.firebaserc` — NOT `indii-music-founder.web.app`, which is the landing target); the live entry chunk `assets/index-k3-WAJdS.js` contains `[Oe.PROJECT_CANVAS]:!0` — the old dev-only default compiled to `!1` in production, so served `!0` proves the post-fix deploy. Gate mapping `["project-canvas"]` present but inert (kill switch only). Regression lock: `featureFlags.test.ts` asserts the flag defaults enabled. Remaining final check per `.agent/REAL_USER_AUTHENTICITY.md`: founder hard-refreshes production, opens a project from the Project List → Project Canvas renders with existing data.

### ISSUE-1446: Raw technical tool errors were founder-wide; subscribers had no error-report channel

- **Status:** ✅ FIXED (2026-09-26 — audience-split execution contract + report_error superpower tool + errorReports rule & tests; rules release rides the next CI deploy-production)
- **Severity:** 🟡 MEDIUM (founder request: "raw errors are for the founder version a.k.a. me; paid subscriptions must not get that feedback, but they all have to be able to report an error")
- **Module:** Agent prompt assembly / superpower tools / Firestore security rules
- **Evidence:** After the ISSUE-1444 honesty hardening, the contract instructed EVERY agent to quote raw tool errors verbatim — meaning subscribers' agents would relay Firestore/internal service errors into paid-user chats. No report_error tool and no errorReports collection existed, so a subscriber had no path to report a failure at all.
- **Impact:** Internal infrastructure detail (DB paths, service names, error codes) leaked into subscriber-facing conversations; conversely, subscriber failures were invisible — nothing was filed anywhere reviewable.
- **Fix:**
  1. `buildExecutionContract(ambition, { founderDiagnostics })` — audience-aware. Founder (`isFounder === true` or `subscriptionTier === 'founder'`, via new `isFounderUserProfile` mirroring `useIsFounderTier`): FOUNDER DIAGNOSTICS MODE — quote raw errors verbatim. Everyone else: SUBSCRIBER ERROR ETIQUETTE — plain-language one-sentence reason, NEVER paste raw error text/codes/stack traces/DB paths/internal service names, technical detail used only internally to retry/route, plus ERROR REPORTS guidance (offer → report_error → give the short report ID). NO PHANTOM ESCALATIONS stays universal. Wired in `buildFullPrompt` from `context.userProfile`; relay EXECUTION_CONTRACT copy made subscriber-safe by default.
  2. New `report_error` superpower tool (every agent): `ErrorReportTools.report_error` files `errorReports/{reportId}` (`er_<ts>_<rand>`) with userId pinned to the caller, agentId, plain-language summary (creator-readable), technical detail (fix-team-only, truncated 5k), surface, status 'open'. Returns the reference ID for the user.
  3. Firestore rule shipped in the same change (ISSUE-1444 prevention): `errorReports` — verified-user create with `ownerCreateIsValid('userId')`, owner/admin read, update/delete hard-blocked (immutable once filed). 7 emulator tests (create/read/spoofed-userId/unauth/immutable/admin-triage).
  4. ToolRiskRegistry: `report_error` builtin write, no approval. Contract variant + tool unit tests added.
- **Acceptance:** Emulator rules suite 264/264 (incl. 7 new errorReports); renderer scoped suites green (builders incl. founder/subscriber variant assertions, ErrorReportTools 4/4, tools accessibility); root typecheck 0 errors. Remaining: founder chat still shows raw errors (regression-verify with a deliberate tool failure); subscriber chat shows plain language + reference ID; admin triage surface for errorReports is a follow-up.
- **Files:** `packages/renderer/src/services/agent/builders/AgentPromptBuilder.ts` (+ test), `packages/renderer/src/services/agent/tools/ErrorReportTools.ts` (new, + test), `packages/renderer/src/services/agent/definitions/SuperpowerTools.ts`, `packages/renderer/src/services/agent/BaseAgent.ts`, `packages/renderer/src/services/agent/ToolRiskRegistry.ts`, `packages/firebase/firestore.rules`, `packages/firebase/src/test/security/firestore.rules.test.ts`, `packages/firebase/src/relay/agentPrompts.ts`
- **Delivery addendum (2026-09-26 /end, ISSUE-1446 triage surface):** Founder-ordered build of the parked triage UI. New founder-gated Admin SDK callables `listErrorReports` + `updateErrorReportStatus` (`packages/firebase/src/functions/security/errorReportCallables.ts` + 8 tests) — the Firestore rules deliberately allow read-all only via the `admin` token claim (browsers never hold it), so triage travels through callables that re-verify founder status server-side from the authoritative user document (`isFounder === true || subscriptionTier === 'founder'`), never from client assertions. Status transitions write `status/reviewedAt/reviewedBy` via Admin SDK; report documents stay client-immutable. Renderer: `ErrorReportTriageService` (thin httpsCallable wrapper) + founder-only "Error Reports" tab in the DevOps dashboard (`ErrorReportsPanel` — status filters, acknowledge/resolve transitions, expandable fix-team detail, real callable failures surfaced verbatim, no fabricated empty states). Validation: callable tests 8/8, devops suites 5/5, firebase lint 0 errors, firebase tsc clean, root typecheck 0 errors. Rides the next CI deploy-production for functions.
...
- **Status:** 🟠 CODE COMPLETE, LIVE-PROVIDER STEPS OUTSTANDING (2026-09-26)
- **Severity:** 🟠 (Road Director chat could never search places or compute distances; Planning map ignored new waypoints)
- **Module:** Touring / Road Manager (renderer + Cloud Functions + agent tools)
- **Delivered this session:** `MapsTools.search_places` wired to the existing secured `findPlaces` callable; NEW `computeDistanceMatrix` callable (`packages/firebase/src/lib/touring.ts`, exported in `packages/firebase/src/index.ts`) with zod caps (10 origins x 10 destinations), App Check + auth + `GOOGLE_MAPS_API_KEY` secret, pure `normalizeDistanceMatrix` mapper, provider-status → HttpsError mapping; `get_place_details` honestly unavailable (no backend) with matching declarations in Road/Hospitality/EventPlanner configs; PlanningTab waypoint editor seeded from the loaded draft + map always mirrors the live waypoint list (fixes "Total Waypoints: 3 but map shows only Detroit+Cleveland" — those stops were the user's own saved draft, not mocks); route-draft Delete (service + hook + ConfirmDialog UI). Validation: renderer agent definitions 96/96, agent tools 362 passed, touring suites 67/67, firebase touring tests 7/7, root typecheck clean, eslint clean.
- **OUTSTANDING (blocking "done"):**
  1. The Distance Matrix API must be enabled for the `GOOGLE_MAPS_API_KEY` key restrictions in GCP console (the 2026-08-18 audit found the client desktop key restricted to Maps JS/Places/Geocoding only). Until enabled, `get_distance_matrix` will surface `MAPS_PROVIDER_DENIED` at runtime. Infra console step — no code change.
  2. `computeDistanceMatrix` deploys via the normal CI functions deploy; verify the deployed function reports real distances in a live Road Director chat (REAL_USER_AUTHENTICITY: no live-user maps validation was performed this session — structural/unit evidence only).
  3. `e2e/road-manager.spec.ts` is STALE vs the current UI (asserts "Initialize Route", "Generated Itinerary", "Run Logistics Check", "Edit Logistics" — none exist; also mocks a `generateItinerary` payload that the client's `isRouteDraftResponse` validator rejects). Pre-existing breakage, NOT introduced here; fix in a dedicated e2e pass.
- **Acceptance for closure:** enabled key + live chat where `search_places` and `get_distance_matrix` return real Google data; Planning tab map mirrors added waypoints on staging; delete-draft works on staging.
