...
- **Status:** 🟡 PARTIALLY FIXED (2026-08-06) — electron + electron-updater patched; react-router-dom and the axios/fast-xml-parser chain deferred, see below
...
- **Fix (partial):** `electron` 41.7.1→41.10.4 and `electron-updater` 6.8.3→6.8.9 (pulling `builder-util-runtime` ≥9.7.0) — both simple patch-level bumps within the existing declared `^` range, verified via `npm run typecheck`. This closes 3 of the 18 named HIGH findings.
- **Deferred — react-router-dom (5 HIGH: open redirect, RSC XSS, SSR constructor injection, route-matching DoS, CSRF bypass):** attempted the `7.18.2` bump. It's exact-pinned in 4 separate `package.json` files (root + `packages/landing`, `packages/renderer`, `packages/firebase`), not just root. Root's pin updated cleanly, but repeated `npm install` passes could not get all four files to converge on `7.18.2` simultaneously in this session — the three workspace-level pins kept reverting to `7.17.0` between install invocations with no corresponding edit from this session. Reverted cleanly back to the all-`7.17.0` baseline rather than leave a half-applied, `invalid:`-flagged lockfile. Root cause not confirmed; leading hypothesis is contention from another concurrent agent process against the same shared `node_modules`/`package-lock.json` in this checkout (see `.agent/skills/error_memory/ERROR_LEDGER.md`, "npm audit fix across a live shared tree" entry added this session) rather than anything wrong with 7.18.2 itself.
- **Deferred — axios (10 bundled HIGH, transitive via `@indii/firebase` → `@googlemaps/google-maps-services-js`) and fast-xml-parser (already override-pinned at `^5.9.3`, needs `^5.10.1`):** adding an `axios` override and bumping the existing `fast-xml-parser` override, then running `npm update axios fast-xml-parser`, *increased* total vulnerabilities from 24 to 35 (moderate 6→18) — `npm update`'s broader resolution pulled in a stray `fast-xml-parser@4.5.7` somewhere below `@remotion/cloudrun`'s `@google-cloud/storage` dependency, undoing the override instead of applying it. Reverted to baseline. A blanket non-force `npm audit fix` also isn't viable as-is: it aborts on an unrelated `ERESOLVE` between `autoprefixer@10.4.22` and `postcss` in the `packages/landing`/`packages/admin-dashboard` Tailwind toolchain before it ever reaches axios.
- **Recommendation for the next attempt:** do this in a dedicated, isolated git worktree (this repo already uses `.codex/worktrees/*` for exactly this reason) rather than the shared primary checkout, specifically because `node_modules`/`package-lock.json` are shared across every branch checked out in the same working directory regardless of git branch isolation. Handle react-router-dom's 4 pin locations and the axios/fast-xml-parser override in separate, individually-verified commits rather than one combined pass, and avoid `npm update` in favor of explicit `npm install <pkg>@<exact-version>` calls, which behaved more predictably in this session.
- **Depends on:** nothing — parallel-safe, independent of ISSUE-1298/1299. The deferred react-router-dom/axios/fast-xml-parser work depends on a clean, uncontended dependency-install environment (see Recommendation above).
