...
## PASS 12 — Finance deep audit (2026-07-03) — REAL MONEY RISK FOUND
...
- **Summary:** `SplitSheetEscrow.tsx` calls the real `createTransfer` Cloud Function via `httpsCallable`, moving actual money to collaborators' Stripe Connect accounts proportional to their split percentage. The component's comment claims "In production, these are loaded from a Firestore 'split_sheets' collection" — but this is aspirational only; confirmed via grep that there is **zero** Firestore code anywhere in the file (no `getDoc`/`setDoc`/`onSnapshot`), and it's mounted standalone in `FinanceDashboard.tsx` with no props and no wrapping persistence layer. Every piece of state — collaborators, escrow amount, and critically the `released` flag — is ephemeral `useState`, gone on remount/refresh.
  ...
  - **Failure mode 2 (double-pay on partial-failure retry):** `handleReleaseFunds` fires all collaborator transfers via `Promise.all` with no idempotency key on `createTransfer`. If 2 of 3 transfers succeed and the 3rd throws (network blip, Stripe rate limit, etc.), `Promise.all` rejects, the whole batch is marked failed, and the UI's natural next step is "Please try again" — but retrying resends transfers 1 and 2 as well, since there's no per-collaborator "already transferred" tracking. This double-pays the collaborators whose transfer already succeeded.
...
### Pass 12 clean bills (verified deep, not pattern-only)
...
## SECOND PASS — four-lens re-audit (double-click races / authorization-IDOR / accessibility / error-state honesty), 2026-07-06

Re-running the full menu cycle with four altered lenses per William's request. Findings below are numbered continuing from 732.
...
- **Summary:** `packages/firebase/src/pod/printful.ts` defines 8 `onCall` functions, and confirmed via grep that **none** reference `context.auth`/`request.auth`, and the file sets **no** `enforceAppCheck`. All 8 are exported and deployed (`packages/firebase/src/index.ts:1724, 1731-1739`). Firebase v2 `onCall` functions are publicly invokable by anyone who knows the project ID (which ships in the public client config) — a `context.auth` check is the ONLY thing gating them, and it's absent. Three of these are acutely dangerous:
  - **`pod_printfulCreateOrder` (line 84):** takes recipient address + line items straight from `req.data` and POSTs a real order to Printful, billed to the platform's Printful account (real money, real physical goods shipped). An attacker can script unbounded order creation → direct, unbounded financial liability.
  - **`pod_printfulGetOrder` (line 113):** interpolates `req.data.orderId` directly into `/orders/{id}` and returns the full order — customer name, shipping address, email (PII). Classic IDOR: enumerate order IDs → harvest every customer's PII.
  - **`pod_printfulCancelOrder` (line 117):** `DELETE /orders/{req.data.orderId}` with no auth/ownership check → anyone can cancel anyone's order (sabotage/griefing, and order-fulfillment denial).
  - The remaining 5 (`GetProducts`/`GetProduct`/`CalculatePrice`/`GetShippingRates`/`GenerateMockup`) are lower-stakes but still consume the paid Printful API using the platform's secret key with no rate-limit gate → quota/cost-abuse vector.
...
- **Firestore rules posture (the real authorization boundary):** 473 match/allow rules, ZERO `allow ... : if true` over-permissive rules, explicit default-deny fallback (`match /{document=**} { allow read, write: if false; }`), and proper `resource.data.userId == request.auth.uid` ownership checks throughout (spot-verified on `products` — sellerId enforced on both update and delete). No IDOR at the Firestore layer despite clients never checking ownership themselves — the rules do it server-side, which is correct.
- **Cloud Functions callable auth:** 42 of 43 callable-defining files reference auth; only `pod/printful.ts` (ISSUE-733) is the gap.
- **Double-click / race protection (money actions):** `MicroLicensingPortal` (`disabled={...|| purchasing}`), `SubmitReleaseModal` (`disabled={...|| submitting}`), and `SplitSheetEscrow` (`disabled={!allSigned || releasing}`) all have proper in-session double-submit guards. ISSUE-720's residual risk is specifically the refresh/retry-persistence gap, not an in-session double-click.
- **Accessibility (main sidebar):** genuinely accessible — visible `<span>` label when expanded, `aria-label` + tooltip when collapsed, `aria-current="page"` on the active item, `aria-label="Main navigation"` on the `<nav>`.
- **Error-state honesty (agent InboxTab, EmailMarketingPanel, and swept catch blocks):** InboxTab is a model of optimistic-UI-with-rollback (mark-read/star/trash all revert state AND toast on failure). EmailMarketingPanel explicitly refuses to fake generated output on failure ("don't plant a canned line and present it as generated" + error toast). No swallowed-error findings — the `catch {}`-style blocks that exist are all legitimate non-critical cases (localStorage quota, expected empty states, non-blocking background sync).

## AUTHORIZATION-LENS BACKEND SWEEP (2026-07-06) — dedicated deep pass on the whole Firebase backend

Focused follow-up to ISSUE-733: press the authorization lens across every backend entry point (HTTP `onRequest` functions, callables, Firestore triggers), checking not just "is there an auth check" but "does it verify OWNERSHIP, not just authentication" (IDOR) and "does it fail closed."
...
### Authorization-sweep clean bills (verified at function level, not just file-level grep)

- **`distributionRecords.ts` (MODEL implementation):** every exported callable (`assignDistributionIdentifier`, `recordDistributionIdentifier`, `recordDistributionAuditEvent`, `requestDistributionTakedown`, `createSftpIngestionRecord`) calls `requireAuth(context)` for authentication AND — critically — the ones acting on a release (`requestDistributionTakedown`, audit event) call `findWritableReleaseRef(releaseId, uid)`, which loads the release and throws `permission-denied` unless `data.userId === uid || data.ownerId === uid || data.createdBy === uid || userCanAccessOrg(data.orgId, uid)`. This is textbook auth+ownership; no IDOR. The takedown (highest IDOR stakes — taking down another user's release) is fully protected. **This is the reference pattern printful.ts (ISSUE-733) should follow.**
- **HTTP `onRequest` webhooks all verify:** `agentStream.ts` (Firebase ID token), `functions/auth/handoff.ts` (`verifyIdToken`), `functions/api/router.ts` (`verifyAuth` → `verifyIdToken` on every route), `pandadocWebhook.ts` (HMAC-SHA256), `stripe/escrow.ts` (HMAC `x-indii-signature`), `stripe/webhookHandler.ts` (Stripe signature). `webhooks/dispatcher.ts` is an OUTBOUND dispatcher (HMAC on send), not an inbound attack surface.
- **MCP server (`mcp/index.ts`):** `cors: origin:true` + no route-level auth looked concerning, BUT `enforceAppCheck: ENFORCE_APP_CHECK` is applied at the function level (line 187, true in production) providing app-attestation, and the only exposed tool is `format_dsp_metadata` — stateless metadata formatting with zero Firestore/admin/external-API/paid access. Low blast radius even if reached. Acceptable.
- **`executeVideoJob` Firestore trigger (`index.ts:441`) + videoJobs rule:** the trigger trusts `data.userId` for cost attribution, which is safe because the `videoJobs` Firestore create rule enforces `request.resource.data.userId == request.auth.uid` (or org membership) — a user cannot create a generation job billed to another user. Billing-attribution attack blocked at the rules layer.
- **`gateway.ts:351`:** correct IDOR guard — `if (data.userId !== userId) throw` (compares request-supplied userId against the authenticated uid), the right way to handle a userId param.
- **Stripe `webhookHandler.ts:147`:** reads `session.metadata.userId` from a signature-verified Stripe webhook payload — trusted source, safe.
...
## USER-REPORTED PRODUCTION ERROR (2026-07-07) — diagnosed via investigate protocol
...
- **`utils/dynamicImport.ts importWithRetry`:** correct and complete — the reference pattern; catches all ChunkLoadError message variants and reloads.
